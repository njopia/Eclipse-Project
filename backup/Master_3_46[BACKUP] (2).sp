...
stock PatchM60Drop()
{
	...
	new Handle:conf = LoadGameConfigFile("l4d2addresses");
		
	if (conf == INVALID_HANDLE)
	{
		LogError("Could not locate l4d2addresses.cfg");
		return;
	}
...
public OnClientPostAdminCheck(client)
{
	if (bMenuOn)
	{
		...
		if (!IsFakeClient(client))
		{
			...
			Format(query, sizeof(query), "INSERT INTO level50_db VALUES level = '%i', WHERE steamid = '%s'", level, steamid);
			PrintToChatAll("Vo dale");
			}
		}
	}
}
...
stock ExpToLevel(leveldata)
{
	...
	for (new level=0; level<=50; level++)
	{
		...
		{
			exp = xp + ((level * 200) + ((level+1) * 200)); //atencion
			xp = exp;
		}
		if (leveldata == level)
		{
					...
					for (new z=1; z<=MaxClients; z++)
					{
						...
						else if (IsSpecialInfected(z) && !IsPlayerGhost(z))
						{
                        				...
                        				if (distance <= 500)
							{
								...
								if (victim <= 0)
								{
									L4D2_StaggerPlayer(z, client, Origin);
								}
								if (IsSurvivor(client) && !IsFakeClient(client) && !bNightmare)
								{
