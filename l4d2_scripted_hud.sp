...
{
    // Intentar obtener datos dinámicos del módulo HUD System Display
    #if defined GetTeamBonusesHUDText
        GetTeamBonusesHUDText(output, size);
        if (output[0] != '\0')
            return;  // Si hay datos del sistema !buy, mostrar solo esos
    #endif

    // Fallback: mostrar información de Tank (original)
    FormatEx(output, size, "\0");
...
void GetHUD3_Text(char[] output, int size)
{
    // Intentar obtener datos dinámicos del módulo HUD System Display
        ...
        if (output[0] != '\0')
            return;  // Si hay datos de deployables, mostrar solo esos
    #endif

    // Fallback: mostrar información de supervivientes (original)
    FormatEx(output, size, "\0");
