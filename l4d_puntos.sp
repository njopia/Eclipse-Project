...
new MapStart	   = 0;
bool IsMuteProcess = false;
bool IsRegProcess  = false;
...
Handle toMute;
Handle toReg;
Handle g_aPlayers[MAXPLAYERS + 1] = { INVALID_HANDLE, ... };	// scp

float  InfGlobalDamage			  = 0.0;
float  InfGlobalActivateDamage	  = 100.0;
...
char g_sBossNames[10][10]		= { "", "smoker", "boomer", "hunter", "spitter", "jockey", "charger", "witch", "tank", "survivor" };
...
Handle		  Timer3						= INVALID_HANDLE;
Handle		  Timer4						= INVALID_HANDLE;
Handle		  Timer5						= INVALID_HANDLE;
Handle		  Timer6						= INVALID_HANDLE;
Handle		  Timer7						= INVALID_HANDLE;
Handle		  Timer8						= INVALID_HANDLE;
Handle		  Timer9						= INVALID_HANDLE;

Handle		  Timer20						= INVALID_HANDLE;
Handle		  Timer21						= INVALID_HANDLE;
Handle		  Timer22						= INVALID_HANDLE;
Handle		  Timer23						= INVALID_HANDLE;
Handle		  Timer24						= INVALID_HANDLE;
Handle		  Timer25						= INVALID_HANDLE;

Handle		  HulkResetTimer				= INVALID_HANDLE;

static Handle fhZombieAbortControl			= INVALID_HANDLE;
static Handle sdkTakeOverZombieBot			= INVALID_HANDLE;
Handle		  g_hGameConf					= INVALID_HANDLE;	 // Game file path for signatures, adresses and offsets.
Handle		  sdkCallVomitPlayer			= INVALID_HANDLE;	 // SDKCall, vomit or puke players.
Handle		  sdkSetBuffer					= INVALID_HANDLE;
Handle		  Forward1						= INVALID_HANDLE;
Handle		  Forward2						= INVALID_HANDLE;
...
Handle		  NormalGlowTimer[MAXPLAYERS + 1];
...
new RoundEnd	 = 0;
new propinfoburn = -1;
float OriginSpeed[MAXPLAYERS + 1];
new propinfoghost;
...
float SurvSpeedUp_Value		   = 1.35;
float InfSpeedUp_Value		   = 1.4;
...
float SurvMassSpeedUp_Value	   = 1.5;
float SurvMassSpeedUp_Time	   = 60.0;

new SurvMassRegen			   = 0;
new SurvMassRegen_Sum		   = 0;
new SurvMassRegen_Cost		   = 110;
float SurvMassRegen_Time	   = 60.0;
...
new HulkHP					   = 7000;
float HulkSpeedUP			   = 1.6;

bool  VictimTimerStarted	   = false;
...
char modules[100][100];
new registeredmodules = 0;

new VictimID		  = 0;
bool AllowHealth;
bool AllowBuy[MAXPLAYERS];
char MapName[30];
char item[MAXPLAYERS + 1][255];
char bought[MAXPLAYERS + 1][64];
...
static char logfilepath_tank[256];
...
public OnPluginStart()
{
	...
	cvar_AdminPlayerCleanLastOnTime = CreateConVar("l4d_puntos_adm_cleanoldplayers", "2", "How many months old players (last online time) will be cleaned. 0 = Disabled");
	cvar_AdminPlayerCleanPlatime	= CreateConVar("l4d_puntos_adm_cleanplaytime", "30", "How many minutes of playtime to not get cleaned from stats. 0 = Disabled");

	LogToFile(logfilepath, "l4d_puntos 12");

	// Config/control Cvars
	cvar_EnableRankVote				= CreateConVar("l4d_puntos_enablerankvote", "1", "Enable voting of team shuffle by player PPM (Points Per Minute)");
	cvar_HumansNeeded				= CreateConVar("l4d_puntos_minhumans", "1", "Minimum Human players before stats will be enabled");
	cvar_UpdateRate					= CreateConVar("l4d_puntos_updaterate", "90", "Number of seconds between Common Infected point earn announcement/update");
	// cvar_AnnounceRankMinChange = CreateConVar("l4d_puntos_announcerankminpoint", "500", "Minimum change to points before rank change announcement");
	cvar_AnnounceRankChange			= CreateConVar("l4d_puntos_announcerank", "1", "Chat announcment for rank change");
	cvar_AnnounceRankChangeIVal		= CreateConVar("l4d_puntos_announcerankinterval", "60", "Rank change check interval");
	cvar_AnnounceMode				= CreateConVar("l4d_puntos_announcemode", "1", "Chat announcment mode. 0 = Off, 1 = Player Only, 2 = Player Only w/ Public Headshots, 3 = All Public");
	cvar_AnnounceToTeam				= CreateConVar("l4d_puntos_announceteam", "2", "Chat announcment team messages to the team only mode. 0 = Print messages to all teams, 1 = Print messages to own team only, 2 = Print messages to own team and spectators only");
	// cvar_AnnounceSpecial = CreateConVar("l4d_puntos_announcespecial", "1", "Chat announcment mode for special events. 0 = Off, 1 = Player Only, 2 = Print messages to all teams, 3 = Print messages to own team only, 4 = Print messages to own team and spectators only", true, 4.0);
	cvar_MedkitMode					= CreateConVar("l4d_puntos_medkitmode", "0", "Medkit point award mode. 0 = Based on amount healed, 1 = Static amount");
	cvar_SiteURL					= CreateConVar("l4d_puntos_siteurl", "", "Community site URL, for rank panel display");
	cvar_RankOnJoin					= CreateConVar("l4d_puntos_rankonjoin", "1", "Display player's rank when they connect. 0 = Disable, 1 = Enable");
	cvar_DisabledMessages			= CreateConVar("l4d_puntos_disabledmessages", "1", "Show 'Stats Disabled' messages, allow chat commands to work when stats disabled. 0 = Hide messages/disable chat, 1 = Show messages/allow chat");
	// cvar_MaxPoints = CreateConVar("l4d_puntos_maxpoints", "500", "Maximum number of points that can be earned in a single map. Normal = x1, Adv = x2, Expert = x3", true, 500.0);
	cvar_DbPrefix					= CreateConVar("l4d_puntos_dbprefix", "", "Prefix for your stats tables");
	// cvar_LeaderboardTime = CreateConVar("l4d_puntos_leaderboardtime", "14", "Time in days to show Survival Leaderboard times");
	cvar_EnableNegativeScore		= CreateConVar("l4d_puntos_enablenegativescore", "1", "Enable point losses (negative score)");
	cvar_FriendlyFireMode			= CreateConVar("l4d_puntos_ffire_mode", "2", "Friendly fire mode. 0 = Normal, 1 = Cooldown, 2 = Damage based");
	cvar_FriendlyFireMultiplier		= CreateConVar("l4d_puntos_ffire_multiplier", "1.5", "Friendly fire damage multiplier (FormulaScore = Damage * Multiplier)");
	cvar_FriendlyFireCooldown		= CreateConVar("l4d_puntos_ffire_cooldown", "10.0", "Time in seconds for friendly fire cooldown");
	cvar_FriendlyFireCooldownMode	= CreateConVar("l4d_puntos_ffire_cooldownmode", "1", "Friendly fire cooldown mode. 0 = Disable, 1 = Player specific, 2 = General");

	// Game mode Cvars
	cvar_Enable						= CreateConVar("l4d_puntos_enable", "1", "Enable/Disable all stat tracking");
	cvar_EnableCoop					= CreateConVar("l4d_puntos_enablecoop", "1", "Enable/Disable coop stat tracking");
	cvar_EnableSv					= CreateConVar("l4d_puntos_enablesv", "1", "Enable/Disable survival stat tracking");
	cvar_EnableVersus				= CreateConVar("l4d_puntos_enableversus", "1", "Enable/Disable versus stat tracking");
	cvar_EnableTeamVersus			= CreateConVar("l4d_puntos_enableteamversus", "1", "[L4D2] Enable/Disable team versus stat tracking");
	cvar_EnableRealism				= CreateConVar("l4d_puntos_enablerealism", "1", "[L4D2] Enable/Disable realism stat tracking");
	cvar_EnableScavenge				= CreateConVar("l4d_puntos_enablescavenge", "1", "[L4D2] Enable/Disable scavenge stat tracking");
	cvar_EnableTeamScavenge			= CreateConVar("l4d_puntos_enableteamscavenge", "1", "[L4D2] Enable/Disable team scavenge stat tracking");
	cvar_EnableRealismVersus		= CreateConVar("l4d_puntos_enablerealismvs", "1", "[L4D2] Enable/Disable realism versus stat tracking");
	cvar_EnableTeamRealismVersus	= CreateConVar("l4d_puntos_enableteamrealismvs", "1", "[L4D2] Enable/Disable team realism versus stat tracking");
	cvar_EnableMutations			= CreateConVar("l4d_puntos_enablemutations", "1", "[L4D2] Enable/Disable mutations stat tracking");

	// Game mode depended Cvars
	cvar_RealismMultiplier			= CreateConVar("l4d_puntos_realismmultiplier", "1.4", "[L4D2] Realism score multiplier for coop score");
	cvar_RealismVersusSurMultiplier = CreateConVar("l4d_puntos_realismvsmultiplier_s", "1.4", "[L4D2] Realism score multiplier for survivors versus score");
	cvar_RealismVersusInfMultiplier = CreateConVar("l4d_puntos_realismvsmultiplier_i", "0.6", "[L4D2] Realism score multiplier for infected versus score");
	cvar_EnableSvMedicPoints		= CreateConVar("l4d_puntos_medicpointssv", "0", "Survival medic points enabled");

	// Infected point Cvars
	cvar_Infected					= CreateConVar("l4d_puntos_infected", "2", "Base score for killing a Common Infected");
	cvar_Hunter						= CreateConVar("l4d_puntos_hunter", "20", "Base score for killing a Hunter");
	cvar_Smoker						= CreateConVar("l4d_puntos_smoker", "20", "Base score for killing a Smoker");
	cvar_Boomer						= CreateConVar("l4d_puntos_boomer", "15", "Base score for killing a Boomer");
	cvar_Spitter					= CreateConVar("l4d_puntos_spitter", "15", "[L4D2] Base score for killing a Spitter");
	cvar_Jockey						= CreateConVar("l4d_puntos_jockey", "15", "[L4D2] Base score for killing a Jockey");
	cvar_Charger					= CreateConVar("l4d_puntos_charger", "25", "[L4D2] Base score for killing a Charger");
	cvar_InfectedDamage				= CreateConVar("l4d_puntos_infected_damage", "2", "The amount of damage inflicted to Survivors to earn 1 point");

	LogToFile(logfilepath, "l4d_puntos 14");

	// Misc personal gain Cvars
	cvar_Pills					   = CreateConVar("l4d_puntos_pills", "15", "Base score for giving Pills to a friendly");
	cvar_Adrenaline				   = CreateConVar("l4d_puntos_adrenaline", "15", "[L4D2] Base score for giving Adrenaline to a friendly");
	cvar_Medkit					   = CreateConVar("l4d_puntos_medkit", "20", "Base score for using a Medkit on a friendly");
	cvar_Defib					   = CreateConVar("l4d_puntos_defib", "20", "[L4D2] Base score for using a Defibrillator on a friendly");
	cvar_SmokerDrag				   = CreateConVar("l4d_puntos_smokerdrag", "5", "Base score for saving a friendly from a Smoker Tongue Drag");
	cvar_JockeyRide				   = CreateConVar("l4d_puntos_jockeyride", "10", "[L4D2] Base score for saving a friendly from a Jockey Ride");
	cvar_ChargerPlummel			   = CreateConVar("l4d_puntos_chargerplummel", "10", "[L4D2] Base score for saving a friendly from a Charger Plummel");
	cvar_ChargerCarry			   = CreateConVar("l4d_puntos_chargercarry", "15", "[L4D2] Base score for saving a friendly from a Charger Carry");
	cvar_ChokePounce			   = CreateConVar("l4d_puntos_chokepounce", "10", "Base score for saving a friendly from a Hunter Pounce / Smoker Choke");
	cvar_Revive					   = CreateConVar("l4d_puntos_revive", "15", "Base score for Revive a friendly from Incapacitated state");
	cvar_Rescue					   = CreateConVar("l4d_puntos_rescue", "10", "Base score for Rescue a friendly from a closet");
	cvar_Protect				   = CreateConVar("l4d_puntos_protect", "3", "Base score for Protect a friendly in combat");
	cvar_PlayerLedgeSuccess		   = CreateConVar("l4d_puntos_ledgegrap", "15", "Base score for causing a survivor to grap a ledge");
	cvar_Matador				   = CreateConVar("l4d_puntos_matador", "30", "[L4D2] Base score for killing a charging Charger with a melee weapon");
	cvar_WitchCrowned			   = CreateConVar("l4d_puntos_witchcrowned", "30", "Base score for Crowning a Witch");

	// Team gain Cvars
	cvar_Panic					   = CreateConVar("l4d_puntos_panic", "25", "Base team score for surviving a Panic Event with no Incapacitations");
	cvar_BoomerMob				   = CreateConVar("l4d_puntos_boomermob", "10", "Base team score for surviving a Boomer Mob with no Incapacitations");
	cvar_SafeHouse				   = CreateConVar("l4d_puntos_safehouse", "10", "Base score for reaching a Safe House");
	cvar_Witch					   = CreateConVar("l4d_puntos_witch", "10", "Base score for Not Disturbing a Witch");
	cvar_VictorySurvivors		   = CreateConVar("l4d_puntos_campaign", "5", "Base score for Completing a Campaign");
	cvar_VictoryInfected		   = CreateConVar("l4d_puntos_infected_win", "30", "Base victory score for Infected Team");

	// Point loss Cvars
	cvar_FFire					   = CreateConVar("l4d_puntos_ffire", "25", "Base score for Friendly Fire");
	cvar_FIncap					   = CreateConVar("l4d_puntos_fincap", "75", "Base score for a Friendly Incap");
	cvar_FKill					   = CreateConVar("l4d_puntos_fkill", "250", "Base score for a Friendly Kill");
	cvar_InSafeRoom				   = CreateConVar("l4d_puntos_insaferoom", "5", "Base score for letting Infected in the Safe Room");
	cvar_Restart				   = CreateConVar("l4d_puntos_restart", "100", "Base score for a Round Restart");
	cvar_MedkitUsedPointPenalty	   = CreateConVar("l4d_puntos_medkitpenalty", "0.0", "Score reduction for all Survivor earned points for each used Medkit (FormulaScore = NormalPoints * (1 - MedkitsUsed * MedkitPenalty))");
	cvar_MedkitUsedPointPenaltyMax = CreateConVar("l4d_puntos_medkitpenaltymax", "1.0", "Maximum score reduction (the score reduction will not go over this value when a Medkit is used)");
	cvar_MedkitUsedFree			   = CreateConVar("l4d_puntos_medkitpenaltyfree", "99", "Team Survivors can use this many Medkits for free without any reduction to the score");
	cvar_MedkitUsedRealismFree	   = CreateConVar("l4d_puntos_medkitpenaltyfree_r", "-1", "Team Survivors can use this many Medkits for free without any reduction to the score when playing in Realism gamemodes (-1 = use the value in l4d_puntos_medkitpenaltyfree)");
	cvar_MedkitBotMode			   = CreateConVar("l4d_puntos_medkitbotmode", "0", "Add score reduction when bot uses a medkit. 0 = No, 1 = Bot uses a Medkit to a human player, 2 = Bot uses a Medkit to other than itself, 3 = Yes");
	cvar_CarAlarm				   = CreateConVar("l4d_puntos_caralarm", "50", "[L4D2] Base score for a Triggering Car Alarm");
	cvar_BotScoreMultiplier		   = CreateConVar("l4d_puntos_botscoremultiplier", "1.0", "Multiplier to use when receiving bot related score penalty. 0 = Disable");

	LogToFile(logfilepath, "l4d_puntos 15");

	// Survivor point Cvars
	cvar_SurvivorDeath				= CreateConVar("l4d_puntos_survivor_death", "40", "Base score for killing a Survivor");
	cvar_SurvivorIncap				= CreateConVar("l4d_puntos_survivor_incap", "15", "Base score for incapacitating a Survivor");

	// Hunter point Cvars
	cvar_HunterPerfectPounceDamage	= CreateConVar("l4d_puntos_perfectpouncedamage", "25", "The amount of damage from a pounce to earn Perfect Pounce (Death From Above) success points");
	cvar_HunterPerfectPounceSuccess = CreateConVar("l4d_puntos_perfectpouncesuccess", "25", "Base score for a successful Perfect Pounce");
	cvar_HunterNicePounceDamage		= CreateConVar("l4d_puntos_nicepouncedamage", "15", "The amount of damage from a pounce to earn Nice Pounce (Pain From Above) success points");
	cvar_HunterNicePounceSuccess	= CreateConVar("l4d_puntos_nicepouncesuccess", "10", "Base score for a successful Nice Pounce");
	cvar_HunterDamageCap			= CreateConVar("l4d_puntos_hunterdamagecap", "25", "Hunter stored damage cap");
	...
	// Boomer point Cvars
	cvar_BoomerSuccess			= CreateConVar("l4d_puntos_boomersuccess", "5", "Base score for a successfully vomiting on survivor");
	cvar_BoomerPerfectHits		= CreateConVar("l4d_puntos_boomerperfecthits", "4", "The number of survivors that needs to get blinded to earn Boomer Perfect Vomit Award and success points");
	cvar_BoomerPerfectSuccess	= CreateConVar("l4d_puntos_boomerperfectsuccess", "30", "Base score for a successful Boomer Perfect Vomit");

	// Tank point Cvars
	cvar_TankDamageCap			= CreateConVar("l4d_puntos_tankdmgcap", "500", "Maximum inflicted damage done by Tank to earn Infected damagepoints");
	cvar_TankDamageTotal		= CreateConVar("l4d_puntos_bulldozer", "200", "Damage inflicted by Tank to earn Bulldozer Award and success points");
	cvar_TankDamageTotalSuccess = CreateConVar("l4d_puntos_bulldozersuccess", "50", "Base score for Bulldozer Award");
	cvar_TankThrowRockSuccess	= CreateConVar("l4d_puntos_tankthrowrocksuccess", "5", "Base score for a Tank thrown rock hit");

	// Charger point Cvars
	cvar_ChargerRamSuccess		= CreateConVar("l4d_puntos_chargerramsuccess", "40", "Base score for a successful Charger Scattering Ram");
	cvar_ChargerRamHits			= CreateConVar("l4d_puntos_chargerramhits", "4", "The number of impacts on survivors to earn Scattering Ram Award and success points");

	// Misc L4D2 Cvars
	cvar_AmmoUpgradeAdded		= CreateConVar("l4d_puntos_deployammoupgrade", "10", "[L4D2] Base score for deploying ammo upgrade pack");
	cvar_GascanPoured			= CreateConVar("l4d_puntos_gascanpoured", "5", "[L4D2] Base score for successfully pouring a gascan");

	LogToFile(logfilepath, "l4d_puntos 17");

	// Other Cvars
	cvar_Top10PPMMin   = CreateConVar("l4d_puntos_top10ppmplaytime", "30", "Minimum playtime (minutes) to show in top10 ppm list");
	cvar_RankVoteTime  = CreateConVar("l4d_puntos_rankvotetime", "20", "Time to wait people to vote");

	cvar_SoundsEnabled = CreateConVar("l4d_puntos_soundsenabled", "1", "Play sounds on certain events");
	...
	StartPoints		   = CreateConVar("l4d2_points_start", "0", "Points to start each round/map with.");
	...
	WitchLimit		   = CreateConVar("l4d2_points_witch_limit", "3", "How many witchs' to be allwed spawned per team");
	...
	PointsSCAR		   = CreateConVar("l4d2_points_scar", "12", "How many points the scar costs");
	...
	PointsEAmmo		   = CreateConVar("l4d2_points_explosive_ammo", "10", "How many points the explosive ammo costs");
	PointsIAmmo		   = CreateConVar("l4d2_points_incendiary_ammo", "10", "How many points the incendiary ammo costs");
	...
	PointsLSight	   = CreateConVar("l4d2_points_laser", "10", "How many points the laser sight costs");
	PointsHeal		   = CreateConVar("l4d2_points_survivor_heal", "25", "How many points a complete heal costs");
	...
	CatRifles		   = CreateConVar("l4d2_points_cat_rifles", "1", "Enable rifles catergory");
	CatSMG			   = CreateConVar("l4d2_points_cat_smg", "1", "Enable smg catergory");
	CatSnipers		   = CreateConVar("l4d2_points_cat_snipers", "1", "Enable snipers catergory");
	CatShotguns		   = CreateConVar("l4d2_points_cat_shotguns", "1", "Enable shotguns catergory");
	CatThrowables	   = CreateConVar("l4d2_points_cat_throwables", "1", "Enable throwables catergory");
	CatMisc			   = CreateConVar("l4d2_points_cat_misc", "1", "Enable misc catergory");
	CatMelee		   = CreateConVar("l4d2_points_cat_melee", "1", "Enable melee catergory");
...
stock PrecacheSoundSmart(char[] sSound)
{
...
public OnClientDisconnect(client)
{
	...
	if (MapTimingStartTime >= 0.0)
	{
		...
		GetClientRankAuthchar(client, ClientID, sizeof(ClientID));
...
public action_DbPrefixChanged(Handle
						   convar,
					   const char[] oldValue,
					   const char[] newValue)
{
...
public action_TimerChanged(Handle
						convar,
					const char[] oldValue,
					const char
						[] newValue)
...
public action_DifficultyChanged(Handle
							 convar,
						 const char
							 [] oldValue,
						 const char
...
// Update the CurrentGamemode when the Cvar is changed.
public action_GamemodeChanged(Handle
						   convar,
					   const char
						   [] oldValue,
					   const char
...
public Action event_ScavengeRoundStart(Handle
									event,
								const char
									[] name,
								bool
...
public Action event_RoundStart(Handle
							event,
						const char
							[] name,
						bool
...
public Action UpdateCostsTimer(Handle
							timer)
{
	UpdateMassCosts();
	...
	if (SQL_CheckConfig(DB_CONF_NAME))
	{
		...
		else if (!SQL_FastQuery(db, "SET NAMES 'utf8'"))
		{
			if (SQL_GetError(db, Error, sizeof(Error)))
				...
				LogError("Failed to update encoding to UTF8unknown");
		}
...
bool CheckDatabaseValidity(const char
							   [] Prefix)
{
	return true;
...
public Action timer_ProtectedFriendly(Handle
								   timer,
							   any
								   data)
{
	...
	GetClientRankAuthchar(data, UserID, sizeof(UserID));
...
public Action timer_InfectedDamageCheck(Handle
									 timer,
								 any
									 data)
{
	...
	if (Score > 0)
	{
		...
		GetClientRankAuthchar(data, iID, sizeof(iID));
...
public Action timer_BoomerBlindnessCheck(Handle
									  timer,
								  any
									  data)
{
...
// Perform player init.
public Action InitPlayers(Handle
					   timer)
{
	if (db == INVALID_HANDLE)
		return;
...
QueryClientPoints(Client, SQLTCallback
							  callback = INVALID_FUNCTION)
{
	char SteamID[MAX_LINE_WIDTH];

	GetClientRankAuthchar(Client, SteamID, sizeof(SteamID));
...
QueryClientRank(Client, SQLTCallback
							callback = INVALID_FUNCTION)
{
	return;
	if (callback == INVALID_FUNCTION)
...
*/
QueryClientGameModeRankDP(Handle
							  dp,
						  SQLTCallback
							  callback)
{
...
QueryClientGameModePointsDP(Handle
								dp,
							SQLTCallback
								callback)
{
...
QueryClientStatsDP(Handle
					   dp)
{
	QueryClientGameModePointsDP(dp, QueryClientStatsDP_1);
}

public QueryClientStatsDP_1(Handle
						 owner,
					 Handle
						 hndl,
					 const char
...
public QueryClientStatsDP_2(Handle
						 owner,
					 Handle
						 hndl,
					 const char
...
public QueryClientStatsDP_3(Handle
						 owner,
					 Handle
						 hndl,
					 const char
...
public QueryClientStatsDP_4(Handle
						 owner,
					 Handle
						 hndl,
					 const char
...
public QueryClientStatsDP_5(Handle
						 owner,
					 Handle
						 hndl,
					 const char
...
public QueryClientStatsDP_6(Handle
						 owner,
					 Handle
						 hndl,
					 const char
...
QueryClientStatsDP_Rank(Client, const char
									[] SteamID)
{
	char query[1024];
...
QueryClientStatsDP_Top10(Client, const char
									 [] SteamID)
{
	char query[1024];
	Format(query, sizeof(query), "SELECT name, %s, %s, kills, versus_kills_survivors + scavenge_kills_survivors + realism_kills_survivors + mutations_kills_survivors, headshots FROM %splayers WHERE steamid = '%s'", DB_PLAYERS_TOTALPLAYTIME, DB_PLAYERS_TOTALPOINTS, DbPrefix, SteamID);
...
QueryClientStatsDP_NextRank(Client, const char
										[] SteamID)
{
	char query[1024];
...
QueryClientStatsDP_NextRankFull(Client, const char
											[] SteamID)
{
	char query[2048];
	Format(query, sizeof(query), "SELECT (%s + 1) - %i FROM %splayers WHERE (%s) >= %i AND steamid <> '%s' ORDER BY (%s) ASC LIMIT 1", DB_PLAYERS_TOTALPOINTS, ClientPoints[Client], DbPrefix, DB_PLAYERS_TOTALPOINTS, ClientPoints[Client], SteamID, DB_PLAYERS_TOTALPOINTS);
...
ReadClientRankMuteSteamID(Client, const char
									  [] SteamID = "")
{
...
public GetClientRankMute(Handle
					  owner,
				  Handle
					  hndl,
				  const char
...
public SetClientRankMute(Handle
					  owner,
				  Handle
					  hndl,
				  const char
...
public Action DelayedMuteFinish(Handle
							 timer,
						 any
							 client)
{
...
public SQLErrorCheckCallback(Handle
						  owner,
					  Handle
						  hndl,
					  const char
...
// Report error on sql query;
public UpdatePlayerCallback(Handle
						 owner,
					 Handle
						 hndl,
					 const char
...
public UpdateMapStat(const char
				  [] Field,
			  Score)
{
	return;
...
// End blinded state.
public Action timer_EndBoomerBlinded(Handle
								  timer,
							  any
								  data)
{
...
// End blinded state.
public Action timer_EndSmokerParalyzed(Handle
									timer,
								any
									data)
{
...
// End lunging state.
public Action timer_EndHunterLunged(Handle
								 timer,
							 any
								 data)
{
...
// End plummel state.
public Action timer_EndChargerPlummel(Handle
								   timer,
							   any
								   data)
{
...
public Action timer_EndCharge(Handle
						   timer,
					   any
						   data)
{
	...
	GetClientRankAuthchar(data, AttackerID, sizeof(AttackerID));
...
// End carried state.
public Action timer_EndChargerCarry(Handle
								 timer,
							 any
								 data)
{
...
public Action timer_EndJockeyRide(Handle
							   timer,
						   any
							   data)
{
...
public Action timer_FriendlyFireDamageEnd(Handle
									   timer,
								   any
									   dp)
{
...
// Start team shuffle.
public Action timer_ShuffleTeams(Handle
							  timer,
						  any
							  data)
{
	...
	for (new i = 1; i <= maxplayers; i++)
	{
		...
		GetClientRankAuthchar(i, SteamID, sizeof(SteamID));
...
// End of RANKVOTE.
public Action timer_RankVote(Handle
						  timer,
					  any
						  data)
{
...
public Action timer_FriendlyFireCooldownEnd(Handle
										 timer,
									 any
										 data)
{
...
// End friendly fire cooldown.
public Action timer_MeleeKill(Handle
						   timer,
					   any
						   data)
{
	...
	char query[512];
	char clientID[MAX_LINE_WIDTH];
	GetClientRankAuthchar(data, clientID, sizeof(clientID));
...
public Action timer_UpdatePlayers(Handle
							   timer,
						   Handle
							   hndl)
{
...
// Display rank change.
public Action timer_ShowRankChange(Handle
								timer,
							any
								client)
{
...
public Action timer_ShowTimerScore(Handle
								timer,
							Handle
								hndl)
{
...
public Action event_PlayerDeath(Handle event, const char[] name, bool dontBroadcast)
{
	...
	char AttackerName[MAX_LINE_WIDTH];
	char AttackerID[MAX_LINE_WIDTH];
	GetClientRankAuthchar(Attacker, AttackerID, sizeof(AttackerID));
	...
	// Team KillAttacker is a Survivor and Victim is Survivor
	...
	else if (AttackerTeam == TEAM_SURVIVORS && VictimTeam == TEAM_INFECTED)
	{
		...
		if (Mode && Score > 0)
		{
			if (GetEventBool(event, "headshot"))
			{
				...
				else if (!ClientRankMute[Attacker]) StatsPrintToChat(Attacker, "You have earned \x04%i \x01points for killing%s \x05%s \x01with a \x04HEAD SHOT\x01!", Score, (VictimIsBot ? " a" : ""), VictimName);
			}
			else
			{
				if (Mode > 2)
...
public Action event_InfectedDeath(Handle
							   event,
						   const char
							   [] name,
						   bool
...
public Action event_TankKilled(Handle
							event,
						const char
							[] name,
						bool
							dontBroadcast)
{
	...
	for (new i = 1; i <= maxplayers; i++)
	{
		...
		{
			GetClientRankAuthchar(i, iID, sizeof(iID));
...
GiveAdrenaline(Giver, Recipient, AdrenalineID = -1)
{
	...
	GetClientRankAuthchar(Recipient, RecipientID, sizeof(RecipientID));

	char GiverName[MAX_LINE_WIDTH];
	GetClientName(Giver, GiverName, sizeof(GiverName));
	char GiverID[MAX_LINE_WIDTH];
	GetClientRankAuthchar(Giver, GiverID, sizeof(GiverID));
...
public Action event_GivePills(Handle
						   event,
					   const char
						   [] name,
					   bool
...
public Action event_DefibPlayer(Handle
							 event,
						 const char
							 [] name,
						 bool
...
// Medkit give code.
public Action event_HealPlayer(Handle
							event,
						const char
							[] name,
						bool
...
// Friendly fire code.
public Action event_FriendlyFire(Handle
							  event,
						  const char
							  [] name,
						  bool
...
// Campaign win code.
public Action event_CampaignWin(Handle
							 event,
						 const char
							 [] name,
						 bool
...
public Action event_MapTransition(Handle
							   event,
						   const char
							   [] name,
						   bool
...
// Begin panic event.
public Action event_PanicEvent(Handle
							event,
						const char
							[] name,
						bool
...
public Action timer_PanicEventEnd(Handle
							   timer,
						   Handle
							   hndl)
{
...
// Begin Boomer blind.
public Action event_PlayerBlind(Handle
							 event,
						 const char
							 [] name,
						 bool
...
public Action event_PlayerBlindEnd(Handle
								event,
							const char
								[] name,
							bool
...
public Action event_PlayerIncap(Handle
							 event,
						 const char
							 [] name,
						 bool
...
public Action event_TongueSave(Handle
							event,
						const char
							[] name,
						bool
...
public Action event_ChokeSave(Handle
						   event,
					   const char
						   [] name,
					   bool
...
public Action event_PounceSave(Handle
							event,
						const char
							[] name,
						bool
...
public Action event_PlayerFallDamage(Handle
								  event,
							  const char
								  [] name,
							  bool
...
public Action event_MeleeKill(Handle
						   event,
					   const char
						   [] name,
					   bool
...
// Player is hanging from a ledge.
public Action event_PlayerLedge(Handle
							 event,
						 const char
							 [] name,
						 bool
							 dontBroadcast)
{
	...
	if ((Score > 0) && (IsValidPlayer(Attacker)))
	{
		...
		char ClientID[MAX_LINE_WIDTH];
		GetClientRankAuthchar(Attacker, ClientID, sizeof(ClientID));
...
public Action event_PlayerSpawn(Handle
							 event,
						 const char
							 [] name,
						 bool
							 dontBroadcast)
{
	...
	if ((IsNormalPlayer(Player)) && (GetClientTeam(Player) == 3) && (IsFakeClient(Player)) && (CurrentGamemodeID == 0))
	{
		...
		if ((chance <= 20) && (!pTank))
		{
			InfSpecialShield[Player] = 1;
			doColor					 = true;
		}
		chance = GetRandomInt(1, 100);
		if ((chance <= 20) && (!pTank))
		{
			InfBonusHealth[Player] = 1;
			doColor				   = true;
		}
		chance = GetRandomInt(1, 100);
		if (chance <= 20)
		...
		if ((chance <= 20) && (!pTank))
		{
			InfFireShield[Player] = 1;
			doColor				  = true;
		}
		chance = GetRandomInt(1, 100);
		if (chance <= 20)
		{
			InfMeeleShield[Player] = 1;
			doColor				   = true;
		}
		chance = GetRandomInt(1, 100);
		if (chance <= 20)
...
public Action DoColorTimer(Handle
						timer,
					int
						client)
{
...
public Action Event_PlayerHurt(Handle
							event,
						const char
							[] name,
						bool
...
// Smoker events.
public Action event_SmokerGrap(Handle
							event,
						const char
							[] name,
						bool
...
// Jockey events.
public Action event_JockeyStart(Handle
							 event,
						 const char
							 [] name,
						 bool
							 dontBroadcast)
{
	...
	GetClientRankAuthchar(Attacker, iID, sizeof(iID));
	if (strlen(iID) > 25) return;
...
public Action event_JockeyRelease(Handle
							   event,
						   const char
							   [] name,
						   bool
							   dontBroadcast)
{
	...
	{
		char query[1024];
		char JockeyName[MAX_LINE_WIDTH];
		char VictimName[MAX_LINE_WIDTH];
		char RescuerName[MAX_LINE_WIDTH];
		char RescuerID[MAX_LINE_WIDTH];
		char UpdatePoints[32];
		new Score = ModifyScoreDifficulty(GetConVarInt(cvar_JockeyRide), 2, 3, TEAM_SURVIVORS);

		GetClientRankAuthchar(Rescuer, RescuerID, sizeof(RescuerID));
...
public Action event_JockeyKilled(Handle
							  event,
						  const char
							  [] name,
						  bool
...
// Charger events.
public Action event_ChargerKilled(Handle
							   event,
						   const char
							   [] name,
						   bool
							   dontBroadcast)
{
	...
	char query[1024];
	char KillerName[MAX_LINE_WIDTH];
	char KillerID[MAX_LINE_WIDTH];
	char UpdatePoints[32];
	new Score	   = 0;
	bool IsMatador = GetEventBool(event, "melee") && GetEventBool(event, "charging");

	GetClientRankAuthchar(Killer, KillerID, sizeof(KillerID));
	...
	if (Mode)
	{
		...
		{
			char VictimName[MAX_LINE_WIDTH];
			char ChargerName[MAX_LINE_WIDTH];
...
public Action event_ChargerCarryStart(Handle
								   event,
							   const char
								   [] name,
							   bool
...
public Action event_ChargerCarryRelease(Handle
									 event,
								 const char
									 [] name,
								 bool
...
public Action event_ChargerImpact(Handle
							   event,
						   const char
							   [] name,
						   bool
...
public Action event_ChargerPummelStart(Handle
									event,
								const char
									[] name,
								bool
...
public Action event_ChargerPummelRelease(Handle
									  event,
								  const char
									  [] name,
								  bool
...
// Hunter events.
public Action event_HunterRelease(Handle
							   event,
						   const char
							   [] name,
						   bool
...
// Smoker events.
public Action event_SmokerRelease(Handle
							   event,
						   const char
							   [] name,
						   bool
...
public Action event_UpgradePackAdded(Handle
								  event,
							  const char
								  [] name,
							  bool
								  dontBroadcast)
{
	...
	GetClientRankAuthchar(Player, PlayerID, sizeof(PlayerID));
...
public Action event_GascanPoured(Handle
							  event,
						  const char
							  [] name,
						  bool
...
114 - Outbreak
*/
public Action event_Achievement(Handle
							 event,
						 const char
							 [] name,
						 bool
...
// Saferoom door opens.
public Action event_DoorOpen(Handle
						  event,
					  const char
						  [] name,
					  bool
...
public Action event_StartArea(Handle
						   event,
					   const char
						   [] name,
					   bool
...
public Action event_PlayerTeam(Handle
							event,
						const char
							[] name,
						bool
...
// AbilityUse.
public Action event_AbilityUse(Handle
							event,
						const char
							[] name,
						bool
							dontBroadcast)
{
	...
	if (!IsClientBot(Player) && GetClientInfectedType(Player) == INF_ID_BOOMER)
	{
		char query[1024];
		char iID[MAX_LINE_WIDTH];
		GetClientRankAuthchar(Player, iID, sizeof(iID));
		if (strlen(iID) > 25) return;
...
// Player got pounced.
public Action event_PlayerPounced(Handle
							   event,
						   const char
							   [] name,
						   bool
...
public Action event_RevivePlayer(Handle
							  event,
						  const char
							  [] name,
						  bool
							  dontBroadcast)
{
	...
	GetClientRankAuthchar(Savior, SaviorID, sizeof(SaviorID));
	if (strlen(SaviorID) > 25) return;

	char VictimName[MAX_LINE_WIDTH];
	GetClientName(Victim, VictimName, sizeof(VictimName));
	char VID[MAX_LINE_WIDTH];
	GetClientRankAuthchar(Victim, VID, sizeof(VID));
...
public Action event_Award_L4D1(Handle
							event,
						const char
							[] name,
						bool
							dontBroadcast)
{
	...
	char UpdatePoints[32];
	char UserID[MAX_LINE_WIDTH];
	GetClientRankAuthchar(User, UserID, sizeof(UserID));
...
// Miscellaneous events and awards. See specific award for info.
public Action event_Award_L4D2(Handle
							event,
						const char
							[] name,
						bool
...
public Action event_ScavengeHalftime(Handle
								  event,
							  const char
								  [] name,
							  bool
...
public Action event_SurvivalStart(Handle
							   event,
						   const char
							   [] name,
						   bool
...
public Action event_CarAlarm(Handle
						  event,
					  const char
						  [] name,
					  bool
...
public Action event_WitchSpawn(Handle
							event,
						const char
							[] name,
						bool
...
public Action CheckWitchDistance(Handle
							  timer,
						  any
							  ent)
{
...
// Witch was crowned!
public Action event_WitchCrowned(Handle
							  event,
						  const char
							  [] name,
						  bool
							  dontBroadcast)
{
	...
	{
		char SteamID[MAX_LINE_WIDTH];
		GetClientRankAuthchar(Killer, SteamID, sizeof(SteamID));
		if (strlen(SteamID) > 25) return;
...
// Witch was disturbed!
public Action event_WitchDisturb(Handle
							  event,
						  const char
							  [] name,
						  bool
...
//	floatReductionFactor = GetMedkitPointReductionFactor();
//
...
-----------------------------------------------------------------------------
*/
public Action HandleCommands(client, const char
								  [] Text)
{
	return Plugin_Continue;
}
...
public Action cmd_ClearRank(client, args)
{
	if (client == 0)
	{
		PrintToConsole(client, "[RANK] Clear StatsDatabase clearing from server console currently disabled because of a bug in it! Run the command from in-game console or from Admin Panel.");
	...
	{
		PrintToConsole(client, "[RANK] Clear StatsStarted clearing the database!");
	...
	PrintToConsole(client, "[RANK] Clear StatsTo clear the database, execute this command again in %.2f seconds!", CLEAR_DATABASE_CONFIRMTIME);
...
public ClearStatsAll(client)
{
	...
	{
		PrintToConsole(client, "[RANK] Clear StatsClearing timedmaps table failed. Executing rollback...");
		DoFastQuery(client, "ROLLBACK");
		PrintToConsole(client, "[RANK] Clear StatsFailure!");
	...
	{
		PrintToConsole(client, "[RANK] Clear StatsClearing players table failed. Executing rollback...");
		DoFastQuery(client, "ROLLBACK");
		PrintToConsole(client, "[RANK] Clear StatsFailure!");
...
public ClearStatsPlayers(client)
{
	...
	else
	{
		...
		PrintToConsole(client, "[RANK] Clear StatsRanks succesfully cleared!");
	}
}

public ClearStatsMapsHandler(Handle
						  owner,
					  Handle
						  hndl,
					  const char
						  ...
						  client)
{
	...
	if (hndl == INVALID_HANDLE)
	{
		PrintToConsole(client, "[RANK] Clear StatsQuery failed! (%s)", error);
		DoFastQuery(client, "ROLLBACK");
		PrintToConsole(client, "[RANK] Clear StatsFailure!");
		return;
	}
	...
	{
		PrintToConsole(client, "[RANK] Clear StatsClearing maps table failed. Executing rollback...");
		DoFastQuery(client, "ROLLBACK");
		PrintToConsole(client, "[RANK] Clear StatsFailure!");
	}
	else
	{
		DoFastQuery(client, "COMMIT");
		PrintToConsole(client, "[RANK] Clear StatsStats succesfully cleared!", query);
	}
}
...
public Action timer_ClearDatabase(Handle
							   timer,
						   any
							   data)
{
...
public DisplayRankMenu(client)
{
	...
	if (GetConVarBool(cvar_EnableRankVote) && (CurrentGamemodeID == GAMEMODE_REALISMVERSUS || CurrentGamemodeID == GAMEMODE_VERSUS && !IsGamemode("teamversus") || CurrentGamemodeID == GAMEMODE_SCAVENGE && !IsGamemode("teamscavenge")))
...
{
	PrintToConsole(0, "[RANK] ErrorMost of the rank commands including this one are not available from server console.");
}
...
public GetClientPointsRankChange(Handle
							  owner,
						  Handle
							  hndl,
						  const char
...
public GetClientGameModePoints(Handle
							owner,
						Handle
							hndl,
						const char
...
public DisplayClientNextRank(Handle
						  owner,
					  Handle
						  hndl,
					  const char
...
// Generate client's next rank.
public GetClientNextRank(Handle
					  owner,
				  Handle
					  hndl,
				  const char
...
public GetClientRankRankChange(Handle
							owner,
						Handle
							hndl,
						const char
...
// Generate client's rank.
public GetClientGameModeRank(Handle
						  owner,
					  Handle
						  hndl,
					  const char
...
public GetGameModeRankTotal(Handle
						 owner,
					 Handle
						 hndl,
					 const char
...
// Send the NEXTRANK panel to the client's display.
public DisplayNextRankFull(Handle
						owner,
					Handle
						hndl,
					const char
				...
				client)
{
	...
	float HeadshotRatio = Headshots == 0 ? 0.00 : FloatDiv(float(Headshots), float(InfectedKilled)) * 100;
...
{
	PrintToConsole(client, "[RANK] Command usagesm_rankmute <0|1>");
}
...
public Action cmd_MapTimes(client, args)
{
	...
	char CurrentMapName[MAX_LINE_WIDTH];
...
public ExecuteTeamShuffle(Handle
					   owner,
				   Handle
					   hndl,
				   const char
					   ...
					   data)
{
	...
	for (i = 1; i <= maxplayers; i++)
	{
		if (IsClientConnected(i) && IsClientInGame(i) && !IsClientBot(i))
		{
			...
			{
				LogError("ExecuteTeamShuffle failed! ReasonDuplicate SteamID while generating shuffled teams.");
	...
	if (GetArraySize(SurvivorArray) > 0 || GetArraySize(InfectedArray) > 0)
	{
		...
		{
			LogError("ExecuteTeamShuffle failed! ReasonTeam size limits block Rank Vote functionality. (Survivors Limit = %i [%i] / Infected Limit = %i [%i])", SurvivorsLimit, NewSurvivorCounter, InfectedLimit, NewInfectedCounter);
			StatsPrintToChatAllPreFormatted("Team shuffle failed in an error.");
					...
					client)
{
	...
	char   Name[32];
	char   DisplayName[MAX_LINE_WIDTH];
...
public CreateTimedMapsMenu(Handle
						owner,
					Handle
						hndl,
					const char
						...
						client)
{
	...
	char   GamemodeInfo[2];	   //, MutationInfo[MAX_LINE_WIDTH];
...
public Menu_CreateTimedMapsMenuHandler(Handle
									menu,
								MenuAction
									action,
								param1, param2)
...
bool TimedMapsMenuInfoMarker(char
								 [] Info,
							 MenuNumber)
{
...
public DisplayTimedMapsMenu2FromInfo(client, char
										  [] Info)
{
...
public CreateTimedMapsMenu2(Handle
						 owner,
					 Handle
						 hndl,
					 const char
						 ...
						 client)
{
	...
	Handle menu = CreateMenu(Menu_CreateTimedMapsMenu2Hndl), Gamemode;
...
public Menu_CreateTimedMapsMenu2Hndl(Handle
								  menu,
							  MenuAction
								  action,
							  param1, param2)
...
public DisplayTimedMapsMenu3FromInfo(client, char
										  [] Info)
{
...
public DisplayTimedMapsMenu3_2(Handle
							owner,
						Handle
							hndl,
						const char
...
public CreateTimedMapsMenu3(Handle
						 owner,
					 Handle
						 hndl,
					 const char
...
public Menu_CreateTimedMapsMenu3Hndl(Handle
								  menu,
							  MenuAction
								  action,
							  param1, param2)
...
public DisplayTimedMapsMenu4FromInfo(client, char
										  [] Info)
{
	...
	if (Position < 0)
	{
		LogError("Timed Maps menu 4 errorInfo = \"%s\"", Info);
		return;
	}
...
public CreateTimedMapsMenu4(Handle
						 owner,
					 Handle
						 hndl,
					 const char
...
public Menu_CreateTimedMapsMenu4Hndl(Handle
								  menu,
							  MenuAction
								  action,
							  param1, param2)
...
public Menu_CreateRanksMenuHandler(Handle
								menu,
							MenuAction
								action,
							param1, param2)
...
public Menu_CreateRankMenuHandler(Handle
							   menu,
						   MenuAction
							   action,
						   param1, param2)
...
public DisplaySettingsPanel(client)
{
	...
	DrawPanelItem(panel, (ClientRankMute[client] ? "Unmute (CurrentlyMuted)" : "Mute (CurrentlyNot muted)"));
...
public RankPanelHandler(Handle
					 menu,
				 MenuAction
					 action,
				 param1, param2)
...
public NextRankPanelHandler(Handle
						 menu,
					 MenuAction
						 action,
					 param1, param2)
...
// Handler for NEXTRANK panel.
public NextRankFullPanelHandler(Handle
							 menu,
						 MenuAction
							 action,
						 param1, param2)
...
public TimedMapsPanelHandler(Handle
						  menu,
					  MenuAction
						  action,
					  param1, param2)
...
// Handler for RANKADMIN panel.
public ClearPlayersPanelHandler(Handle
							 menu,
						 MenuAction
							 action,
						 param1, param2)
...
// Handler for RANKADMIN panel.
public ClearMapsPanelHandler(Handle
						  menu,
					  MenuAction
						  action,
					  param1, param2)
...
// Handler for RANKADMIN panel.
public ClearAllPanelHandler(Handle
						 menu,
					 MenuAction
						 action,
					 param1, param2)
...
// Handler for RANKADMIN panel.
public CleanPlayersPanelHandler(Handle
							 menu,
						 MenuAction
							 action,
						 param1, param2)
...
// Handler for RANKADMIN panel.
public RemoveCustomMapsPanelHandler(Handle
								 menu,
							 MenuAction
								 action,
							 param1, param2)
...
// Handler for RANKADMIN panel.
public ClearTMAllPanelHandler(Handle
						   menu,
					   MenuAction
						   action,
					   param1, param2)
...
// Handler for RANKADMIN panel.
public ClearTMCoopPanelHandler(Handle
							menu,
						MenuAction
							action,
						param1, param2)
...
// Handler for RANKADMIN panel.
public ClearTMSurvivalPanelHandler(Handle
								menu,
							MenuAction
								action,
							param1, param2)
...
// Handler for RANKADMIN panel.
public ClearTMRealismPanelHandler(Handle
							   menu,
						   MenuAction
							   action,
						   param1, param2)
...
// Handler for RANKADMIN panel.
public ClearTMMutationsPanelHandler(Handle
								 menu,
							 MenuAction
								 action,
							 param1, param2)
...
public RankVotePanelHandler(Handle
						 menu,
					 MenuAction
						 action,
					 param1, param2)
...
// Handler for RANKADMIN panel.
public DisplayClearPanelHandler(Handle
							 menu,
						 MenuAction
							 action,
						 param1, param2)
...
public AboutPanelHandler(Handle
					  menu,
				  MenuAction
					  action,
				  param1, param2)
...
public SettingsPanelHandler(Handle
						 menu,
					 MenuAction
						 action,
					 param1, param2)
...
public Top10PanelHandler(Handle
					  menu,
				  MenuAction
					  action,
				  param1, param2)
...
public Top10PPMPanelHandler(Handle
						 menu,
					 MenuAction
						 action,
					 param1, param2)
...
{
	// ToDouse cvar_MaxPoints to check if the score is within the map limits
...
public UpdateSmokerDamage(Client, Damage)
{
	...
	char iID[MAX_LINE_WIDTH];
	GetClientRankAuthchar(Client, iID, sizeof(iID));
...
UpdateJockeyRideLength(Client, float
								   RideLength = -1.0)
{
...
CheckSurvivorsAllDown()
{
	...
	if (Score > 0)
		for (i = 0; i < InfectedCounter; i++)
		{
			...
			GetClientRankAuthchar(Infected[i], ClientID, sizeof(ClientID));
	...
	for (i = 0; i < IncapCounter; i++)
	{
		...
		GetClientRankAuthchar(KilledSurvivor[i], ClientID, sizeof(ClientID));
...
UpdateTankSniperSteamID(const char
							[] ClientID)
{
	char query[1024];
...
float GetMedkitPointReductionFactor()
{
	...
	// If Penalty is set to ZEROThere is no reduction.
	if (Penalty <= 0.0)
		return 1.0;
...
GetMedkitPointReductionScore(Score, bool
										ToCeil = false)
{
...
public StartMapTiming()
{
	...
	for (new i = 1; i <= maxplayers; i++)
	{
		if (IsClientConnected(i) && IsClientInGame(i) && !IsClientBot(i))
		{
			...
			if (ClientTeam == TEAM_SURVIVORS)
			{
				GetClientRankAuthchar(i, ClientID, sizeof(ClientID));
...
public UpdateMapTimingStat(Handle
						owner,
					Handle
						hndl,
					const char
...
GetClientRankAuthchar(client, char[] auth, maxlength)
{
...
public StatsPrintToChatAll(const char
						[] Message,
					any...)
{
	char FormattedMessage[MAX_MESSAGE_WIDTH];
...
public StatsPrintToChatAllPreFormatted(const char
									[] Message)
{
	new maxplayers = MaxClients;
...
public StatsPrintToChatPreFormatted(Client, const char
										 [] Message)
{
...
bool UpdateServerSettings(Client, const char[] Key, const char[] Value, const char[] Desc)
{
	...
	{
		PrintToConsole(Client, "[RANK] %sSetting a new MOTD value failure!", Desc);
		return false;
	}
	...
	{
		PrintToConsole(Client, "[RANK] %sUpdate failed! (ReasonCannot create SQL statement)");
		return false;
	}
	...
	if (!SQL_Execute(statement))
	{
		if (SQL_GetError(db, error, sizeof(error)))
		{
			PrintToConsole(Client, "[RANK] %sUpdate failed! (Error = \"%s\")", Desc, error);
			LogError("%sUpdate failed! (Error = \"%s\")", error, Desc);
		}
		else
		{
			PrintToConsole(Client, "[RANK] %sUpdate failed!", Desc);
			LogError("%sUpdate failed!", Desc);
		}
	...
	{
		PrintToConsole(Client, "[RANK] %sUpdate successful!", Desc);
		ShowMOTDAll();
	}
...
public PS_RegisterModule(Handle
					  plugin,
				  numParams)
{
	char test[100];
...
public PS_UnregisterModule(Handle
						plugin,
					numParams)
{
	char container[100];
	bool found = false;	   // might remove later it might not be good with multiple instances of the same module. though hopefully I can avoid that
	for (new i = 0; i <= 99; i++)
	{
		if (found) return;
...
public PS_SetItem(Handle
			   plugin,
		   numParams)
{
	char newchar[100];
	new client = GetNativeCell(1);
	GetNativeString(2, newchar, sizeof(newchar));
	Format(item[client], sizeof(item), newchar);
}

public PS_SetCost(Handle
			   plugin,
		   numParams)
{
	new client	 = GetNativeCell(1);
			 ...
			 numParams)
{
	...
	new client = GetNativeCell(1);
	GetNativeString(2, newchar, sizeof(newchar));
	Format(bought[client], sizeof(bought), newchar);
}

public PS_SetBoughtCost(Handle
					 plugin,
				 numParams)
{
	new client		   = GetNativeCell(1);
...
public PS_GetBoughtCost(Handle
					 plugin,
				 numParams)
{
	new client = GetNativeCell(1);
...
public OnClientAuthorized(client, const char
							   [] auth)
{
	if (points[client] > GetConVarInt(StartPoints)) return;
	points[client] = GetConVarInt(StartPoints);
...
public Action Event_Finale(Handle
						event,
					char
						[] event_name,
					bool
...
public Action Event_Kill(Handle
					  event,
				  const char
					  [] name,
				  bool
...
stock AddWeaponAmmo(client, ammo)
{
	...
	{												   // caseMilitary Sniper Rifle or CSS Snipers
		offsettoadd = MILITARY_SNIPER_OFFSET_IAMMO;	   // gun type specific offset
	}
	else
	{	 // caseno gun this plugin recognizes
		return Plugin_Continue;
	}
...
public Action Event_Incap(Handle
					   event,
				   const char
					   [] name,
				   bool
...
//}
public Action Event_TankDeath(Handle
						   event,
					   const char
						   [] name,
					   bool
...
public Action Event_WitchDeath(Handle
							event,
						const char
							[] name,
						bool
...
public Action Event_Heal(Handle
					  event,
				  const char
					  [] name,
				  bool
...
public Action Event_Protect(Handle
						 event,
					 const char
						 [] name,
					 bool
...
public Action Event_Revive(Handle
						event,
					const char
						[] name,
					bool
...
public Action Event_Shock(Handle
					   event,
				   const char
					   [] name,
				   bool
...
public Action Event_Choke(Handle
					   event,
				   const char
					   [] name,
				   bool
...
public Action Event_Boom(Handle
					  event,
				  const char
					  [] name,
				  bool
...
public Action Event_Pounce(Handle
						event,
					const char
						[] name,
					bool
...
public Action Event_Ride(Handle
					  event,
				  const char
					  [] name,
				  bool
...
public Action Event_Carry(Handle
					   event,
				   const char
					   [] name,
				   bool
...
public Action Event_Impact(Handle
						event,
					const char
						[] name,
					bool
...
public Action Event_Burn(Handle
					  event,
				  const char
					  [] name,
				  bool
...
public Action Event_Hurt(Handle
					  event,
				  const char
					  [] name,
				  bool
...
public Action Command_RBuy(client, args)
{
	...
	if (args > 0)
	{
		ReplyToCommand(client, "[SM] Usagesm_repeatbuy");
		return Plugin_Handled;
	}
	if (client == 0)
	{
	...
	if (args == 0 && client > 0 && IsClientInGame(client))
	{
		...
		else if (StrEqual(item[client], "give ammo", false))
		{
			...
			new m60ammo	 = 150;
			new nadeammo = 30;
			Handle cvar	 = FindConVar("l4d2_guncontrol_m60ammo");
			Handle cvar2 = FindConVar("l4d2_guncontrol_grenadelauncherammo");
...
public Action Command_Heal(client, args)
{
	if (args > 1)
	{
		ReplyToCommand(client, "[SM] Usagesm_heal <targetsmlib");
		return Plugin_Handled;
	}
	if (args == 0)
	{
...
public Action Command_Points(client, args)
{
	if (args < 1 || args > 2)
	{
		ReplyToCommand(client, "[SM] Usagesm_givepoints <#userid|name> [number of points]");
		return Plugin_Handled;
	}
...
public Action Command_SPoints(client, args)
{
	if (args < 1 || args > 2)
	{
		ReplyToCommand(client, "[SM] Usagesm_setpoints <#userid|name> [number of points]");
		return Plugin_Handled;
	}
	char arg[MAX_NAME_LENGTH];
	char arg2[4];
...
BuildBuyMenu(client)
{
	...
	if (GetClientTeam(client) == 2)
	{
		...
		Handle menu = CreateMenu(MenuHandler_Survivors);
		// SetMenuExitBackButton(menu, true);
	...
	if (GetClientTeam(client) == 3)
	{
		...
		char   mob[40];
		char   umob[40];
		Handle menu = CreateMenu(InfectedMenu);
...
BuildMeleeMenu(client)
{
	...
	char crowbar[40];
	char tonfa[40];
	char baseball_bat[40];
	char cricket_bat[40];
	char electric_guitar[40];
	char golfclub[40];
	char katana[40];
	char frying_pan[40];
	char machete[40];
	...
	{
		Handle menu = CreateMenu(MenuHandler_Melee);
...
{
	char   hunting_rifle[40];
	char   title[40];
	char   sniper_military[40];
	char   sniper_scout[40];
	char   sniper_awp[40];
	Handle menu = CreateMenu(MenuHandler_Snipers);
...
BuildRiflesMenu(client)
{
	char   rifle[40];
	char   title[40];
	char   rifle_desert[40];
	char   rifle_ak47[40];
	char   rifle_sg552[40];
	char   rifle_m60[40];
	Handle menu = CreateMenu(MenuHandler_Rifles);
...
{
	char   autoshotgun[40];
	char   shotgun_chrome[40];
	char   shotgun_spas[40];
	char   pumpshotgun[40];
	char   title[40];
	Handle menu = CreateMenu(MenuHandler_Shotguns);
...
BuildSMGMenu(client)
{
	char   smg[40];
	char   title[40];
	char   smg_silenced[40];
	char   smg_mp5[40];
	Handle menu = CreateMenu(MenuHandler_SMG);
...
{
	char   adrenaline[40];
	char   defibrillator[40];
	char   first_aid_kit[40];
	char   pain_pills[40];
	char   health[40];
	char   title[40];
	Handle menu = CreateMenu(MenuHandler_Health);
...
BuildThrowablesMenu(client)
{
	...
	char   vomitjar[40];
	char   title[40];
	Handle menu = CreateMenu(MenuHandler_Throwables);
...
{
	char   grenade_launcher[40];
	char   fireworkcrate[40];
	char   gascan[40];
	char   oxygentank[40];
	char   propanetank[40];
	char   pistol[40];
	char   pistol_magnum[40];
	char   title[40];
	char   gnome[40];
	char   cola_bottles[40];
	char   chainsaw[40];
	Handle menu = CreateMenu(MenuHandler_Misc);
...
{
	char   upgradepack_explosive[40];
	char   upgradepack_incendiary[40];
	char   title[40];
	char   laser_sight[40];
	char   explosive_ammo[40];
	char   incendiary_ammo[40];
	char   ammo[40];
	Handle menu = CreateMenu(MenuHandler_Upgrades);
...
public MenuHandler_Melee(Handle
					  menu,
				  MenuAction
					  action,
				  param1, param2)
...
public MenuHandler_Rifles(Handle
					   menu,
				   MenuAction
					   action,
				   param1, param2)
...
public MenuHandler_Snipers(Handle
						menu,
					MenuAction
						action,
					param1, param2)
...
public MenuHandler_Shotguns(Handle
						 menu,
					 MenuAction
						 action,
					 param1, param2)
...
public MenuHandler_Throwables(Handle
						   menu,
					   MenuAction
						   action,
					   param1, param2)
...
public MenuHandler_Misc(Handle
					 menu,
				 MenuAction
					 action,
				 param1, param2)
...
public MenuHandler_Health(Handle
					   menu,
				   MenuAction
					   action,
				   param1, param2)
...
public MenuHandler_Upgrades(Handle
						 menu,
					 MenuAction
						 action,
					 param1, param2)
...
DisplayConfirmMenuMelee(param1)
{
	...
	char   no[40];
	char   title[40];
	Handle menu = CreateMenu(MenuHandler_ConfirmMelee);
...
DisplayConfirmMenuSMG(param1)
{
	...
	char   no[40];
	char   title[40];
	Handle menu = CreateMenu(MenuHandler_ConfirmSMG);
	Format(yes, sizeof(yes), "Да");
	AddMenuItem(menu, "yes", yes);
	Format(no, sizeof(no), "Нет");
	AddMenuItem(menu, "no", no);
...
DisplayConfirmMenuRifles(param1)
{
	...
	char   no[40];
	char   title[40];
	Handle menu = CreateMenu(MenuHandler_ConfirmRifles);
	Format(yes, sizeof(yes), "Да");
	AddMenuItem(menu, "yes", yes);
	Format(no, sizeof(no), "Нет");
	AddMenuItem(menu, "no", no);
...
DisplayConfirmMenuSnipers(param1)
{
	...
	char   no[40];
	char   title[40];
	Handle menu = CreateMenu(MenuHandler_ConfirmSniper);
	Format(yes, sizeof(yes), "Да");
	AddMenuItem(menu, "yes", yes);
	Format(no, sizeof(no), "Нет");
	AddMenuItem(menu, "no", no);
...
DisplayConfirmMenuShotguns(param1)
{
	...
	char   no[40];
	char   title[40];
	Handle menu = CreateMenu(MenuHandler_ConfirmShotguns);
	Format(yes, sizeof(yes), "Да");
	AddMenuItem(menu, "yes", yes);
	Format(no, sizeof(no), "Нет");
	AddMenuItem(menu, "no", no);
...
DisplayConfirmMenuThrow(param1)
{
	...
	char   no[40];
	char   title[40];
	Handle menu = CreateMenu(MenuHandler_ConfirmThrow);
	Format(yes, sizeof(yes), "Да");
	AddMenuItem(menu, "yes", yes);
	Format(no, sizeof(no), "Нет");
	AddMenuItem(menu, "no", no);
...
DisplayConfirmMenuMisc(param1)
{
	...
	char   no[40];
	char   title[40];
	Handle menu = CreateMenu(MenuHandler_ConfirmMisc);
	Format(yes, sizeof(yes), "Да");
	AddMenuItem(menu, "yes", yes);
	Format(no, sizeof(no), "Нет");
	AddMenuItem(menu, "no", no);
...
DisplayConfirmMenuUpgrades(param1)
{
	...
	char   no[40];
	char   title[40];
	Handle menu = CreateMenu(MenuHandler_ConfirmUpgrades);
	Format(yes, sizeof(yes), "Да");
	AddMenuItem(menu, "yes", yes);
	Format(no, sizeof(no), "Нет");
	AddMenuItem(menu, "no", no);
...
DisplayConfirmMenuI(param1)
{
	...
	else if (StrContains(item[param1], "hulk") != -1) {
		ShowDistance[param1] = 1;
...
public MenuHandler_ConfirmMelee(Handle
							 menu,
						 MenuAction
							 action,
						 param1, param2)
...
public MenuHandler_ConfirmRifles(Handle
							  menu,
						  MenuAction
							  action,
						  param1, param2)
...
public MenuHandler_ConfirmSniper(Handle
							  menu,
						  MenuAction
							  action,
						  param1, param2)
...
public MenuHandler_ConfirmSMG(Handle
						   menu,
					   MenuAction
						   action,
					   param1, param2)
...
public MenuHandler_ConfirmShotguns(Handle
								menu,
							MenuAction
								action,
							param1, param2)
...
public MenuHandler_ConfirmThrow(Handle
							 menu,
						 MenuAction
							 action,
						 param1, param2)
...
public MenuHandler_ConfirmMisc(Handle
							menu,
						MenuAction
							action,
						param1, param2)
...
public MenuHandler_ConfirmHealth(Handle
							  menu,
						  MenuAction
							  action,
						  param1, param2)
...
public MenuHandler_ConfirmUpgrades(Handle
								menu,
							MenuAction
								action,
							param1, param2)
...
public MenuHandler_ConfirmI(Handle
						 menu,
					 MenuAction
						 action,
					 param1, param2)
...
public Action AllowBuyTimer(Handle
						 timer,
					 any
						 client)
{
...
public Action AllowHealthTimer(Handle
							timer,
						any
							client)
{
...
public Action Event_PlayerHurt_Pre(Handle
								event,
							const char
								[] name,
							bool
...
EfficientKiller_DamageAdd(iAtt, iVic, iTA, iType, char[] stWpn, iDmgOrig)
{
	new iDmgAdd = 7;
...
public Action Event_lunge_pounce(Handle
							  event,
						  char
							  [] event_name,
						  bool
...
public Action Event_pounce_end(Handle
							event,
						char
							[] event_name,
						bool
...
public Action Event_jockey_ride(Handle
							 event,
						 char
							 [] event_name,
						 bool
...
public Action Event_jockey_ride_end(Handle
								 event,
							 char
								 [] event_name,
							 bool
...
public Action Event_tongue_grab(Handle
							 event,
						 char
							 [] event_name,
						 bool
...
public Action Event_tongue_release(Handle
								event,
							char
								[] event_name,
							bool
...
public Action Event_charger_pummel_start(Handle
									  event,
								  char
									  [] event_name,
								  bool
...
public Action Event_charger_pummel_end(Handle
									event,
								char
									[] event_name,
								bool
...
public Action RemoveSpecialShield(Handle
							   timer,
						   any
							   client)
{
...
public Action RemoveIncSpecialShield(Handle
								  timer,
							  any
								  client)
{
...
public Action ApplyAcidDamage(Handle
						   timer,
					   any
						   client)
{
		...
		if (IsNormalPlayer(i))
		{
			...
			if ((GetClientTeam(i) == 2) && (AcidDamage[i] > 0) && (!IsPlayerIncapped(i)))
			{
				...
				if (FinalHealth > 0)
				{
					...
					if (IsValidPlayer(i))
					{
						PrintCenterText(i, "%t", "acidDamageHint");
...
public event_PlayerIncap2(Handle
					   event,
				   const char
					   [] name,
				   bool
...
char GetName(client)
{
	char name[MAX_NAME_LENGTH] = "noname";
...
public Action ResetSlowDown(Handle
						 timer,
					 any
						 client)
{
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	{
		PrintHintText(victim, "%t", "shieldBlocked2Hint", damage);
	...
	{
		float wasdamage = damage;
		if (VictimTeam == 2)
		{
			damage = FloatMul(damage, 0.2);
			AnimateBlock(victim, 255, 0, 130);
			PrintHintText(victim, "%t", "shieldBlocked1Hint", (wasdamage - damage), wasdamage);
		}
		else {
			AnimateBlock(victim, 255, 0, 130);
			damage = 0.0;
	...
	if ((IsNormalPlayer(attacker)) && (IsPlayerAlive(attacker)))
	{
		...
		if ( (IsValidPlayer(attacker)) && (IsNormalPlayer(victim)) && (GetClientTeam(victim) == 3) ) {
			...
			LogToFile(logfilepath, "inflictor_posx%2.2f y%2.2f z%2.2f", pos[0], pos[1], pos[2]);
			LogToFile(logfilepath, "victim_posx%2.2f y%2.2f z%2.2f", tpos[0], tpos[1], tpos[2]);
			...
			&& (StrContains(name, "grenade_launcher_projectile", false) != -1) && (StrContains(wep_name, "grenade_launcher", false) != -1))
		{
			...
			//}
			/*
			float gl_radius = GetConVarFloat(FindConVar("grenadelauncher_radius_kill"));
	...
	if (((SurvPhysPower > 0) || (victim == VictimID)) && (VictimTeam == 2))
	{
		...
		// FloatTochar(FDamage, SDamage, sizeof(SDamage));
...
public Action event_RoundEnd(Handle
						  event,
					  const char
						  [] name,
					  bool
	...
	if (RoundEnd == 1)
	{
		...
		while ((common = FindEntityByClassname(common, "infected")) != INVALID_ENT_REFERENCE)
		{
			if (IsValidEntity(common))
			{
				...
				if (strcmp(sTemp, "models/infected/common_male_ceda.mdl") == 0 || strcmp(sTemp, "models/infected/common_male_clown.mdl") == 0 || strcmp(sTemp, "models/infected/common_male_fallen_survivor.mdl") == 0 || strcmp(sTemp, "models/infected/common_male_jimmy.mdl") == 0 || strcmp(sTemp, "models/infected/common_male_mud.mdl") == 0 || strcmp(sTemp, "models/infected/common_male_riot.mdl") == 0 || strcmp(sTemp, "models/infected/common_male_roadcrew.mdl") == 0)
				{
...
public ResetClientInfAbilites(any
						   client)
{
	InfSpeedUp[client]		 = 0;
...
public ResetClientSurvAbilites(any
							client)
{
...
public Action event_PlayerTeam2(Handle
							 event,
						 const char
							 [] name,
						 bool
							 dontBroadcast)
{
	...
	new NewTeam		  = GetEventInt(event, "team");
	new OldTeam		  = GetEventInt(event, "oldteam");
	bool IsDisconnect = GetEventBool(event, "disconnect");
...
public MenuHandler_Survivors(Handle
						  menu,
					  MenuAction
						  action,
					  param1, param2)
...
*/
public Action UpdateSpeedUp(Handle
						 Timer,
					 any
						 client)
{
...
public Action UpdateInfSpeedUp(Handle
							Timer,
						any
							client)
{
...
public Action RemoveSurvSpecialShield(Handle
								   timer,
							   any
								   client)
{
...
public Action CheckSurvSpecialShield(Handle
								  timer,
							  any
								  client)
{
...
public Action OnVomited(Handle
					 hEvent,
				 char
					 [] event_name,
				 bool
...
{
	float flMaxDistance = float(radius);
	float power			= power;
	new tcount			= 0;
	...
	if (GetClientTeam(client) == 2)
	{
		...
		for (new i = 1; i <= MaxClients; i++)
		{
			...
			// if (FloatCompare(distance, flMaxDistance) == -1) PrintToChat(client, "%sdistance ok (%i)", GetName(i), FloatCompare(distance, flMaxDistance)); else PrintToChat(client, "%sdistance false (%i)", GetName(i), FloatCompare(distance, flMaxDistance));
			// if (distance <= flMaxDistance) PrintToChat(client, "2 %sdistance ok (%i)", GetName(i), FloatCompare(distance, flMaxDistance)); else PrintToChat(client, "2 %sdistance false (%i)", GetName(i), FloatCompare(distance, flMaxDistance));
			// if (!IsTank(i))  PrintToChat(client, "%snot tank", GetName(i)); else  PrintToChat(client, "%sis tank", GetName(i));
...
public Action SetIsLoadingFalse(Handle
							 timer,
						 any
							 client)
{
...
public Event_RoundFreezeEnd(Handle
						 event,
					 const char
						 [] name,
					 bool
...
public SendPointsHandler(Handle
					  menu,
				  MenuAction
					  action,
				  param1, param2)
{
	...
	Handle menu2 = CreateMenu(SendPointsFinaleHandler);
...
public Action SetBonusHealth(Handle
						  timer,
					  any
						  client)
{
...
public Action SetHulkHealth(Handle
						 timer,
					 any
						 client)
{
...
public Action SetTankChaosHealth(Handle
							  timer,
						  any
							  client)
{
...
public Action YellTimerStop(Handle
						 timer,
					 any
						 client)
{
...
public Action YellTimer(Handle
					 timer,
				 any
					 client)
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		{
			if (IsPlayerIncapped(i)) IsInc[i] = 1;
			else IsInc[i] = 0;
			/*
				new char:CurClassname[255];
				...
				param1)
{
	...
	if (GetClientTeam(param1) == 2)
	{
		...
		{
			new awp_limit;
			if (CurrentGamemodeID != 1) awp_limit = 4;
			else awp_limit = 2;
		...
		{
			new m60_limit;
			if (CurrentGamemodeID != 1) m60_limit = 3;
			else m60_limit = 1;
		...
		{
			new gl_limit;
			if (CurrentGamemodeID != 1) gl_limit = 2;
			else gl_limit = 1;
			if (SurvGL >= gl_limit)
			{
		...
		if (StrEqual(item[param1], "suicide", false))
		{
			...
			float DeathTime = GetRandomFloat(2.0, 6.0);
				 ...
				 CMD[192])
{
	...
	if (GetClientTeam(param1) == 2)
	{
		...
		else if (StrEqual(CMD, "SurvSendPoints", false))
		{
			...
			Handle menu = CreateMenu(SendPointsHandler);
		...
		else if (StrEqual(CMD, "InfBonusDamage", false))
		{
			...
			if (IsTank(param1)) cost[param1] = 60;
			else cost[param1] = InfBonusDamage_Cost;
		}
		...
		else if (StrEqual(CMD, "InfAcidClaws", false))
		{
			...
			if (IsTank(param1)) cost[param1] = 60;
			else cost[param1] = InfAcidClaws_Cost;
		}
		...
		else if (StrEqual(CMD, "InfMeeleShield", false))
		{
			...
			if (IsTank(param1)) cost[param1] = 50;
			else cost[param1] = InfMeeleShield_Cost;
		}
		else if (StrEqual(CMD, "InfRegen", false))
		{
			item[param1] = "InfRegen";
			if (IsTank(param1)) cost[param1] = 100;
			else cost[param1] = InfRegen_Cost;
		}
...
public Action timerEndEffect(Handle
						  timer,
					  any
						  client)
{
...
public Action CreateParticle(target, char[] particlename, float time, float origin)
{
	if (target > 0)
	{
		new particle = CreateEntityByName("info_particle_system");
...
public Action AttachParticle(target, char[] particlename, float time, float origin)
{
...
public Action PrecacheParticle(char
							[] particlename)
{
	new particle = CreateEntityByName("info_particle_system");
	if (IsValidEntity(particle))
...
public Action DeleteParticles(Handle
						   timer,
					   any
						   particle)
{
...
public SendPointsFinaleHandler(Handle
							menu,
						MenuAction
							action,
						param1, param2)
				  ...
				  client)
{
	...
	if (IsTank(client)) CCost = 60;
	else CCost = InfBonusDamage_Cost;
	...
	if (IsTank(client)) CCost = 50;
	else CCost = InfMeeleShield_Cost;
	...
	if (IsTank(client)) CCost = 100;
	else CCost = InfRegen_Cost;
...
//	}
// }
public Action ResetFrustration(Handle
							Timer,
						any
							client)
{
...
public Action RevertPlayerStatus(Handle
							  timer)
{
...
stock SetPlayerIsAlive(client, bool
								   alive)
{
...
stock SetPlayerGhostStatus(client, bool
									   ghost)
{
	if (ghost)
	{
...
stock SetPlayerLifeState(client, bool
									 ready)
{
...
stock CheatCommand(client, const char[] command, const char[] arguments = "")
{
				  ...
				  client)
{
	...
	if (GetClientTeam(client) == 2)
	{
		...
		if (SurvMassSpeedUp > 0) Format(Active, sizeof(Active), "[A]");
		else Format(Active, sizeof(Active), "");
		...
		if (SurvZombieSurprize > 0) Format(Active, sizeof(Active), "[A]");
		else Format(Active, sizeof(Active), "");
		...
		if (SurvUntouchable > 0) Format(Active, sizeof(Active), "[A]");
		else Format(Active, sizeof(Active), "");
		...
		if (InfMassSlow > 0) Format(Active, sizeof(Active), "[A]");
		else Format(Active, sizeof(Active), "");
		...
		if (InfDeathCloud > 0) Format(Active, sizeof(Active), "[A]");
		else Format(Active, sizeof(Active), "");
		...
		if (InfZombieApoc > 0) Format(Active, sizeof(Active), "[A]");
		else Format(Active, sizeof(Active), "");
...
public SendPointsToBank(client)
{
	...
	Handle menu = CreateMenu(SendPointsToBankHandler);
...
public SendPointsToBankHandler(Handle
							menu,
						MenuAction
							action,
						param1, param2)
{
	...
	if (StrEqual(item[param1], "SurvMassSpeedUp", false)) SurvMassSpeedUp_Sum = SurvMassSpeedUp_Sum + PointsValue[param1];
		...
		if (CurrentGamemodeID != 1) SurvAutoMiniGun_Limit = 2;
		else SurvAutoMiniGun_Limit = 1;
...
public CheckBankActivate()
{
	...
	if (((GetTeamHumanCount(3) > 0) || (CurrentGamemodeID != 1)) && (InfAllowMass != 0))
	{
		...
		{
			Handle il = FindConVar("l4d_infected_limit");
			...
			for (new i = 1; i <= MaxClients; i++)
			{
				if (IsNormalPlayer(i))
				{
					if ((GetClientTeam(i) == 2) && (IsPlayerAlive(i)))
					{
						...
						/*
						Handle hBf = StartMessageOne("Shake", i);
		...
		if (InfZombieApoc_Sum >= InfZombieApoc_Cost)
		{
			...
			new tclient = 0;
			for (new i = 1; ((i <= MaxClients) && (tclient == 0)); i++)
...
public Action UncommonHorde(Handle
						 timer,
					 any
						 HordeType)
{
...
public Action CreateAirstrike(Handle
						   timer,
					   any
						   client)
{
...
public Action DeathCloudDamageTimer(Handle
								 timer)
{
	new Health;
...
public Action DeathCloudResetTimer(Handle
								timer,
							any
								data)
{
...
public Action InfPoisonResetTimer(Handle
							   timer,
						   any
							   data)
{
...
public Action ShakeTimer(Handle
					  timer,
				  any
					  data)
{
...
public Action SurvPhysPowerResetTimer(Handle
								   timer,
							   any
								   data)
{
...
public Action SurvUntouchableResetTimer(Handle
									 timer,
								 any
									 data)
{
...
public Action SurvZombieSurprizeResetTimer(Handle
										timer,
									any
										data)
{
...
public Action SurvMassRegenResetTimer(Handle
								   timer,
							   any
								   data)
{
...
public Action SurvMassSpeedUpResetTimer(Handle
									 timer,
								 any
									 data)
{
...
public Action InfMassSlowResetTimer(Handle
								 timer,
							 any
								 data)
{
...
public Action InfMassArmorResetTimer(Handle
								  timer,
							  any
								  data)
{
...
public Action SpeedLogicTimer(Handle
						   timer,
					   any
						   data)
{
...
public Action Event_PlayerShoved(Handle
							  event,
						  const char
							  [] name,
						  bool
...
public Action Event_EntityShoved(Handle
							  event,
						  const char
							  [] name,
						  bool
...
public Action StartTankChaosTimer(Handle
							   timer)
{
	float pos[3];
	float tpos[3];
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if ((IsValidPlayer(i)) && (GetClientTeam(i) == 3) && (IsPlayerGhost(i)))
		{	 //&& (IsPlayerAlive(i))
			...
			for (new j = 1; ((j <= MaxClients) && (!FoundInRange)); j++)
			{
				// if (InfTankChaos <= 0) return;
			...
			//панелька
			Handle TankChaosPanel = CreatePanel();
...
public Action ChaosTankResetTimer(Handle
							   timer,
						   any
							   client)
{
...
public Action SpawnTankTimer(Handle
						  timer,
					  any
						  client)
{
...
static CreateGasCloud(client, float
								  g_pos[3])
{
	float  targettime = GetEngineTime() + 10;
...
public Action Point_Hurt(Handle
					  timer,
				  Handle
					  hurt)
{
	...
	float distance;
	float radiussetting = 250;
	char  soundFilePath[256];
	Format(soundFilePath, sizeof(soundFilePath), "player/survivor/voice/choke_5.wav");
	bool shakeenabled = true;
	new damage		  = 1;
	bool slowenabled  = true;
...
public Action timer_stock_applyDamage(Handle
								   timer,
							   Handle
								   dataPack)
{
							...
							targetposition[3])
{
	...
	{
		LogError("Tracer BugPlayer-Zombie Trace did not hit anything, WTF");
		isVisible = true;
	}
	CloseHandle(trace);
...
public Action StopShake(Handle
					 timer,
				 any
					 target)
{
...
public Action ClearMeleeBlock(Handle
						   timer,
					   any
						   target)
{
...
public Action TeleportToFirstTank(Handle
							   timer,
						   any
							   iCid)
{
...
public Action BlockRocks(Handle
					  timer,
				  any
					  iCid)
{
...
public UpdateMassCosts()
{
	...
	if (sc <= 3) SurvMassSpeedUp_Cost = 190;
	else SurvMassSpeedUp_Cost = (sc * 65) - (sc * 10);
	if (sc <= 3) SurvMassRegen_Cost = 250;
	else SurvMassRegen_Cost = (sc * 80) - (sc * 10);
	if (sc <= 3) SurvAutoMiniGun_Cost = 200;
	else SurvAutoMiniGun_Cost = (sc * 65) - (sc * 10);
	if (sc <= 3) SurvZombieSurprize_Cost = 220;
	else SurvZombieSurprize_Cost = (sc * 70) - (sc * 10);
	if (sc <= 3) SurvUntouchable_Cost = 180;
	else SurvUntouchable_Cost = (sc * 60) - (sc * 10);
	if (sc <= 3) SurvPhysPower_Cost = 200;
	else SurvPhysPower_Cost = (sc * 75) - (sc * 10);
	if (sc <= 3) SurvVictimShield_Cost = 180;
	else SurvVictimShield_Cost = (sc * 60) - (sc * 10);

	if (ic <= 3) InfMassArmor_Cost = 220;
	else InfMassArmor_Cost = (ic * 70) - (ic * 10);
	if (ic <= 3) InfMassSlow_Cost = 200;
	else InfMassSlow_Cost = (ic * 65) - (ic * 10);
	if (ic <= 3) InfDeathCloud_Cost = 180;
	else InfDeathCloud_Cost = (ic * 60) - (ic * 10);
	if (ic <= 3) InfZombieApoc_Cost = 190;
	else InfZombieApoc_Cost = (ic * 65) - (ic * 10);
	if (ic <= 3) InfPoison_Cost = 170;
	else InfPoison_Cost = (ic * 50) - (ic * 10);

	if (ic <= 3) InfBummerRain_Cost = 160;
	else InfBummerRain_Cost = (ic * 60) - (ic * 10);
	if (ic <= 3) InfTankChaos_Cost = 200;
	else InfTankChaos_Cost = (ic * 80) - (ic * 10);
}

public Action SetTankHP(any
					 client)
{
	if (!IsNormalPlayer(client)) return;
...
public Action SetVictim(Handle
					 timer,
				 any
					 client)
{
...
public Action VictimRegen(Handle
					   timer,
				   any
					   client)
{
...
public Action VictimRender(Handle
						timer,
					any
						client)
{
...
public Action cmd_showtankdamage(client, args)
{
	if (ShowTankDamage)
	{
		...
		PrintToChat(client, "ShowTankDamageFalse");
	}
	else {
		ShowTankDamage = true;
		PrintToChat(client, "ShowTankDamageTrue");
	}
}
...
public Action CheckBankTimer(Handle
						  timer,
					  any
						  client)
{
...
public Action CheckTanksTimer(Handle
						   timer,
					   any
						   client)	  // Escudo vip
{
			...
			if (GetClientTeam(i) == 2)
			{
				...
				{	 // Si el jugador es vip2
					...
					{										 // Si no es victima
						...
						// PrintToChat(i, "\x01%t\x04%t %t10", "Activated", "SpecialShield", "disponible");
						// PrintToChat(i, "\x01%t\x04%t 2 %t5", "Activated", "SpecialShield", "disponible");
					}
				}
					...
					{	 // Si el juego es coop
						if (VictimID != i)
						{									 // Si el jugador no es victima
							SurvIncSpecialShield[i] = 10;	 // Escudo especial 2 para vip3
							SurvSpecialShield[i]	= 15;	 // Escudo especial 1 para vip3

							// PrintToChat(i, "\x01%t\x04%t 2  \x01%t20", "Activated", "SpecialShield", "disponible");
							// PrintToChat(i, "\x01%t\x04%t \x01%t30", "Activated", "SpecialShield", "disponible");
						}
						...
						{									 // Si no es victima
							...
							// PrintToChat(i, "\x01%t\x04%t 2  \x01%t5", "Activated", "SpecialShield", "disponible");
							// PrintToChat(i, "\x01%t\x04%t \x01%t10", "Activated", "SpecialShield", "disponible");
						}
...
public Action CheckClientTankChaos(Handle
								timer,
							any
								client)
{
...
public Action CheckMinigunTimer(Handle
							 timer,
						 any
							 data)
{
...
public Event_FinaleVehicleLeaving(Handle
							   event,
						   const char
							   [] name,
						   bool
				  ...
				  client)
{
	...
	{
		IsRegProcess = false;
		LogToFile(logfilepath, "regnamequery2-1 failed %i %s", client, GetName(client));
		return;
	}
	...
	{
		IsRegProcess = false;
		LogToFile(logfilepath, "regnamequery2-2 failed2 %i %s", client, GetName(client));
		return;
	}

	LogToFile(logfilepath, "regnamequery2-3 %i %s", client, GetName(client));
...
/* public regnamequery2(Handle:owner, Handle:hndl, const String:error[], any:client)
{
	...
	{		
		...
		if (VipStatusDisabled[client] == 1) {
			VipStatusWas[client] = VipStatus[client];
			VipStatus[client] = 1;
			PrintToChatAll("\x01Игрок \x04%s \x01изменил VIP статус на \x05%i", GetName(client), VipStatus[client]);
		}
	...
	PrintToChat(client, "Данные о VIP статусе получены.");
	
	CreateTimer(1.0, DelayedRegFinish, client);
}
 */
public Action DelayedRegFinish(Handle
							timer,
						any
							client)
{
...
public Event_Tank_Spawn(Handle
					 event,
				 const char
					 [] name,
				 bool
					 dontBroadcast)
{
	...
	if (InfHulk[iCid] == 1)
	{
		...
		// floatscale = GetEntPropFloat(iCid, Prop_Send, "m_flModelScale");
...
public Action SetTankHPTimer(Handle
						  timer,
					  any
						  client)
{
...
public Action TankSpawnCheck(Handle
						  timer,
					  any
						  client)
{
...
public Action event_heal_end(Handle
						  event,
					  const char
						  [] name,
					  bool
...
public Action CheckClientHPTimer(Handle
							  timer,
						  any
							  client)
{
...
{
	InfPoison = 0;
	for (new i = 1; ((i <= MaxClients) && (InfPoison == 0)); i++)
	{
...
public Action ForceSuicideTimer(Handle
							 timer,
						 any
							 client)
{
...
public Action AllowActivateBuyClientResetTimer(Handle
											timer,
										any
											client)
{
...
public TankChaosPanelHandler(Handle
						  menu,
					  MenuAction
						  action,
					  param1, param2)
...
public Action PounceTimer(Handle
					   timer,
				   any
					   client)
{
	...
	else if ((GetClientTeam(client) == 2) && (SurvSpecialShield[client] > 0) && ((PounceTime[client] > 5) || (IsPounced[client] == 0))) {
		SurvSpecialShield[client] -= 1;
		PrintToChat(client, "\x04%t \x03%i", "shieldrem", SurvSpecialShield[client]);
		return;
	}
	else if ((GetClientTeam(client) == 3) && (InfSpecialShield[client] > 0) && ((PounceTime[client] > 5) || (IsPounced[client] == 0))) {
...
CreateEnvSprite(client, const char
							[] sColor)
{
...
public Event_Player_Class(Handle
					   event,
				   const char
					   [] name,
				   bool
				...
				param1, param2)
{
	...
	if (GetClientTeam(param1) == 2)
	{
		...
		// floatdistance = GetDistance(param1, 800);
		// if (FloatCompare(distance, -1.0) != 0) {
		//		PrintToChat(param1, "\x05%t.", "ToClose");
		//		return;
		//	}
...
public Action cmd_spawnshield(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "[SM] Usagesm_spawnshield <#userid|name>");
		return Plugin_Handled;
	}
	char arg[MAX_NAME_LENGTH];
	char arg2[255];
	...
	float x = StringToFloat(arg2);
	float y = StringToFloat(arg3);
	float z = StringToFloat(arg4);
...
AttachEnt(owner, ent, char[] positon = "medkit", float pos[3] = NULL_VECTOR, float ang[3] = NULL_VECTOR)
{
	char tname[60];
...
public Action cmd_killshield(client, args)
{
	if (args != 1)
	{
		ReplyToCommand(client, "[SM] Usagesm_killshield <#userid|name>");
		return Plugin_Handled;
	}
	char arg[MAX_NAME_LENGTH];
	char arg2[4];
...
public player_bot_replace(Handle
					   Spawn_Event,
...
public bot_player_replace(Handle
					   Spawn_Event,
				   const char[] Spawn_Name,
				   bool
					   Spawn_Broadcast)
...
PushCommon(common, client)
{
	...
	float vAng[3];
	float vPos[3];
	decl  g_fConfTeslaForce	 = 500;
	decl  g_fConfTeslaForceZ = 500;
...
stock bool makeexplosion(attacker = 0, inflictor = -1, const float attackposition[3], const char[] weaponname = "", magnitude = 800, radiusoverride = 0, float damageforce = 0.0, flags = 0)
{
...
PushPoint(client, ent, force, radius)
{
	...
	PrintToChat(client, "point_pushStrForce %s, StrRadius %s, StrTarget %s  successfully created", StrForce, StrRadius, StrTarget);

	CreateTimer(0.5, DeletePushForce, push);
}

public Action DeletePushForce(Handle
						   timer,
					   any
						   ent)
{
...
GetEntDistance(ent, flMaxDistance)
{
	if (!IsValidEntity(ent))
	{
		// PrintToChatAll("GetEntDistancenot valid ent");
		return -1;
	}
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsNormalPlayer(i))
		{
			if (GetClientTeam(i) == 2)
			{
				...
				// PrintToChatAll("GetEntDistancedistance%f", distance);
				// if (FloatCompare(distance, flMaxDistance) == -1) {
				if (distance < flMaxDistance)
				{
					return distance;
	...
	// PrintToChatAll("GetEntDistanceclients not found");
	return -1;
}

public Action ShowDistanceTimer(Handle
							 timer,
						 any
							 client)
{
	...
	// if (FloatCompare(distance, -1.0) == 0)
	if (distance == -1)
		PrintHintText(client, "%tOK", "distance", distance);
...
public Action timer_stock_applyDamage2(Handle
									timer,
								Handle
									dataPack)
{
...
public Action HulkAllowResetTimer(Handle
							   timer)
{
...
public Action ShowTankInfoTimer(Handle
							 timer,
						 any
							 client)
{
...
public Action Event_TankFrustrated(Handle
								event,
							const char
								[] name,
							bool
...
public Action ResetTankChaosEvent(Handle
							   timer)
{
	TankChaosEvent = 0;
}

public Action WitchAllowReset(Handle
						   timer,
					   any
						   client)
{
...
public Action cmd_isnormalalt(client, args)
{
	...
	{
		PrintToChat(client, "isnormalalttrue");
	}
	else PrintToChat(client, "isnormalaltfalse");
}
...
public RoundStartInit()
{
	...
	LogToFile(logfilepath, "RoundStartInitstart");
	...
	LogToFile(logfilepath, "RoundStartInit1");
	...
	LogToFile(logfilepath, "RoundStartInit2");
	...
	LogToFile(logfilepath, "RoundStartInit3");
	...
	LogToFile(logfilepath, "RoundStartInitend");
}

CreateEnvSprite2(client, const char[] sColor, const char[] scale)
{
...
public Action Event_Player_Use(Handle
							event,
						const char
							[] name,
						bool
...
public Action RemoveUpgradeTimer(Handle
							  timer,
						  any
							  client)
{
...
public Action event_Weapon_Reload(Handle
							   event,
						   const char
							   [] name,
						   bool
...
public Action NormalGlow(Handle
					  timer,
				  any
					  client)
{
...
public Action CheckRegArrayTimer(Handle
							  timer)
{
	if (RoundEnd > 0) return;
...
public VipWeaponsMenu(client, w_type)
{
	...
	Handle menu = CreateMenu(VipMenuWeaponsHandler);
	char   text[255];
	char   title[40];
...
public VipMenuWeaponsHandler(Handle
						  menu,
					  MenuAction
						  action,
					  param1, param2)
...
public Action event_InfectedHurt(Handle
							  event,
						  const char
							  [] name,
						  bool
							  dontBroadcast)
{
	...
	char wep_name[40];
	GetClientWeapon(Attacker, wep_name, sizeof(wep_name));
	if ((SurvFirearmsMaster[Attacker] > 0) && ((StrContains(wep_name, "sniper_awp", false) != -1) || (StrContains(wep_name, "sniper_scout", false) != -1)))
	{
...
public Action HurtInfTimer(Handle
						timer,
					Handle
						dataPack)
{
...
public Action Event_SpecialAmmo(Handle
							 event,
						 const char
							 [] name,
						 bool
...
public Action Event_ExplAmmo(Handle
						  event,
					  const char
						  [] name,
					  bool
...
public Action Event_IncAmmo(Handle
						 event,
					 const char
						 [] name,
					 bool
...
public Action cmd_showmass(client, args)
{
	...
	float CSpeed = GetEntDataFloat(client, g_flLagMovement);
...
{
	PrintToChatAll("llegamos al viplist");
	for (new i = 1; i <= MaxClients; i++)
	{
		PrintToChatAll("bucle");
		if ((IsValidPlayer(i)) && (IsValidPlayer(client)))
		{
			PrintToChatAll("llegamos al viplist %i", VipStatus[i]);
			if (VipStatus[i] >= 1) PrintToChat(client, "\x04%s \x01VIP Status\x05%i", GetName(i), VipStatus[i]);
		}
	}
}

public Action ResetShieldAllowTimer(Handle
								 timer,
							 any
								 client)
{
