...
#include <sdkhooks>
#include <basecomm>
#include <eclipse_ms>	 // Incluir el archivo .inc
// ====================================================================================================
// Pragmas
// ====================================================================================================
#pragma semicolon 1
...
public void OnPluginStart()
{
	...
	g_hCvar_HUD1_X			 = CreateConVar("l4d2_scripted_hud_hud1_x", "0.02", "X (horizontal) position of the text.\nNote: setting it to less than 0.0 may cut/hide the text at screen.", CVAR_FLAGS, true, -1.0, true, 1.0);
	g_hCvar_HUD1_Y			 = CreateConVar("l4d2_scripted_hud_hud1_y", "0.015", "Y (vertical) position of the text.\nNote: setting it to less than 0.0 may cut/hide the text at screen.", CVAR_FLAGS, true, -1.0, true, 1.0);
	...
	g_hCvar_HUD1_Width		 = CreateConVar("l4d2_scripted_hud_hud1_width", "1.5", "Text area Width.", CVAR_FLAGS, true, 0.0, true, 2.0);
	...
	g_hCvar_HUD2_X			 = CreateConVar("l4d2_scripted_hud_hud2_x", "0.75", "X (horizontal) position of the text.\nNote: setting it to less than 0.0 may cut/hide the text at screen.", CVAR_FLAGS, true, -1.0, true, 1.0);
	g_hCvar_HUD2_Y			 = CreateConVar("l4d2_scripted_hud_hud2_y", "0.015", "Y (vertical) position of the text.\nNote: setting it to less than 0.0 may cut/hide the text at screen.", CVAR_FLAGS, true, -1.0, true, 1.0);
	...
	g_hCvar_HUD2_Width		 = CreateConVar("l4d2_scripted_hud_hud2_width", "1.5", "Text area Width.", CVAR_FLAGS, true, 0.0, true, 2.0);
	...
	g_hCvar_HUD3_X			 = CreateConVar("l4d2_scripted_hud_hud3_x", "0.02", "X (horizontal) position of the text.\nNote: setting it to less than 0.0 may cut/hide the text at screen.", CVAR_FLAGS, true, -1.0, true, 1.0);
	g_hCvar_HUD3_Y			 = CreateConVar("l4d2_scripted_hud_hud3_y", "0.15", "Y (vertical) position of the text.\nNote: setting it to less than 0.0 may cut/hide the text at screen.", CVAR_FLAGS, true, -1.0, true, 1.0);
	...
	// Admin Commands
	RegAdminCmd("sm_rh", CmdReloadData, ADMFLAG_ROOT, "Reload the HUD texts set in the data file.");
	RegAdminCmd("sm_ph", CmdPrintCvars, ADMFLAG_ROOT, "Print the plugin related cvars and their respective values to the console.");

	CreateTimer(1.0, TimerAliveTankCheck, _, TIMER_REPEAT);
	// Verificar que el plugin Eclipse MS esté cargado
	if (!LibraryExists("eclipse_ms"))
	{
		SetFailState("El plugin Eclipse Management System no está cargado!");
	}
}

public void OnLibraryRemoved(const char[] name)
{
	if (StrEqual(name, "eclipse_ms"))
	{
		SetFailState("Eclipse Management System se ha descargado!");
	}
}
/****************************************************************************************************/
...
void GetHUD_Texts()
{
	...
	// ===== CORRECCIÓN PARA HUD3 - MOSTRAR INFORMACIÓN POR CLIENTE =====
	g_sBuffer			   = "\0";
	...
	{
		// NUEVO: Construir texto para todos los jugadores conectados
		char allPlayersText[512];
		allPlayersText[0] = '\0';
		...
		for (int client = 1; client <= MaxClients; client++)
		{
			...
			// Obtener texto para este cliente específico
			char clientText[128];
			GetHUD3_Text_Test(clientText, sizeof(clientText), client);

			// Agregar al texto total (un jugador por línea)
			if (allPlayersText[0] == '\0')
				FormatEx(allPlayersText, sizeof(allPlayersText), "%s", clientText);
			else
				Format(allPlayersText, sizeof(allPlayersText), "%s\n%s", allPlayersText, clientText);
		}

		FormatEx(g_sBuffer, sizeof(g_sBuffer), "%s%s", allPlayersText, g_sSpaces);
		g_sHUD_TextArray[HUD3] = g_sBuffer;
	}
...
void GetHUD2_Text(char[] output, int size)
{
	...
	for (int client = 1; client <= MaxClients; client++)
	{
		...
		else
		{
			...
			{
				case L4D2_ZOMBIECLASS_SMOKER: zombieClass = "Smoker";
				case L4D2_ZOMBIECLASS_BOOMER: zombieClass = "Boomer";
				case L4D2_ZOMBIECLASS_HUNTER: zombieClass = "Hunter";
				case L4D2_ZOMBIECLASS_SPITTER: zombieClass = "Spitter";
				case L4D2_ZOMBIECLASS_JOCKEY: zombieClass = "Jockey";
				case L4D2_ZOMBIECLASS_CHARGER: zombieClass = "Charger";
				case L4D2_ZOMBIECLASS_TANK: zombieClass = "Tank";
			}

			if (output[0] == 0)
				FormatEx(output, size, "%N [%s]: %s", client, zombieClass, health);
			else
 ...
 */
void GetHUD3_Text_Test(char[] output, int size, int client)
{
	// Limpiar el output
	output[0] = '\0';
	...
	// Obtener datos del sistema Eclipse usando los natives
	int level	   = EMS_GetPlayerLevel(client);
	int currency   = EMS_GetPlayerCurrency(client);
	int currentXP  = EMS_GetPlayerCurrentXP(client);
	int requiredXP = EMS_GetXPForNextLevel(client);
	int progress   = EMS_GetLevelProgress(client);

	// Formatear la salida con toda la información
	FormatEx(output, size, "%s - Lv%d | $%d | XP:%d/%d (%d%%)",
			 playerName, level, currency, currentXP, requiredXP, progress);
}
/****************************************************************************************************/
...
Action CmdPrintCvars(int client, int args)
{
	...
	PrintToConsole(client, "l4d2_scripted_hud_hud1_text_align : %i (%s)", g_iCvar_HUD1_TextAlign, g_iCvar_HUD1_TextAlign == HUD_TEXT_ALIGN_LEFT ? "LEFT" : g_iCvar_HUD1_TextAlign == HUD_TEXT_ALIGN_CENTER ? "CENTER"
																																																		   : "RIGHT");
	...
	PrintToConsole(client, "l4d2_scripted_hud_hud1_team : %i (%s)", g_iCvar_HUD1_Team, g_iCvar_HUD1_Team == HUD_TEAM_ALL ? "ALL" : g_iCvar_HUD1_Team == HUD_TEAM_SURVIVOR ? "SURVIVOR"
																																										  : "INFECTED");
	...
	PrintToConsole(client, "l4d2_scripted_hud_hud2_text_align : %i (%s)", g_iCvar_HUD2_TextAlign, g_iCvar_HUD2_TextAlign == HUD_TEXT_ALIGN_LEFT ? "LEFT" : g_iCvar_HUD2_TextAlign == HUD_TEXT_ALIGN_CENTER ? "CENTER"
																																																		   : "RIGHT");
	...
	PrintToConsole(client, "l4d2_scripted_hud_hud2_team : %i (%s)", g_iCvar_HUD2_Team, g_iCvar_HUD2_Team == HUD_TEAM_ALL ? "ALL" : g_iCvar_HUD2_Team == HUD_TEAM_SURVIVOR ? "SURVIVOR"
																																										  : "INFECTED");
	...
	PrintToConsole(client, "l4d2_scripted_hud_hud3_text_align : %i (%s)", g_iCvar_HUD3_TextAlign, g_iCvar_HUD3_TextAlign == HUD_TEXT_ALIGN_LEFT ? "LEFT" : g_iCvar_HUD3_TextAlign == HUD_TEXT_ALIGN_CENTER ? "CENTER"
																																																		   : "RIGHT");
	...
	PrintToConsole(client, "l4d2_scripted_hud_hud3_team : %i (%s)", g_iCvar_HUD3_Team, g_iCvar_HUD3_Team == HUD_TEAM_ALL ? "ALL" : g_iCvar_HUD3_Team == HUD_TEAM_SURVIVOR ? "SURVIVOR"
																																										  : "INFECTED");
	...
	PrintToConsole(client, "l4d2_scripted_hud_hud4_text_align : %i (%s)", g_iCvar_HUD4_TextAlign, g_iCvar_HUD4_TextAlign == HUD_TEXT_ALIGN_LEFT ? "LEFT" : g_iCvar_HUD4_TextAlign == HUD_TEXT_ALIGN_CENTER ? "CENTER"
																																																		   : "RIGHT");
	...
	PrintToConsole(client, "l4d2_scripted_hud_hud4_team : %i (%s)", g_iCvar_HUD4_Team, g_iCvar_HUD4_Team == HUD_TEAM_ALL ? "ALL" : g_iCvar_HUD4_Team == HUD_TEAM_SURVIVOR ? "SURVIVOR"
																																										  : "INFECTED");
