...
// Duracion: 60 segundos — Cooldown: 5 minutos
//==================================================
...
#define SPEEDFREAK_HP_RESTORE 100	  // HP minimo al restaurar
 ...
 *
 * @param client    Indice del cliente.
 * @return          true si se activo correctamente.
 */
bool	   Ability_SpeedFreak_Activate(int client)
{
	...
	// Aplicar velocidad via speed manager con duracion automatica
	...
	// Particula en el jugador
	FX_AttachParticle(client, PARTICLE_ADRENALINE, SPEEDFREAK_DURATION);

	PrintToChat(client, "\x04[Speed Freak]\x01 Velocidad maxima! HP reducido a \x04%d\x01.", SPEEDFREAK_HP);
 ...
 * Puede llamarse antes de que expire el timer (por muerte, desconexion, etc.).
 *
 * @param client    Indice del cliente.
 */
void Ability_SpeedFreak_Deactivate(int client)
{
	...
	// Remover capa de velocidad — el manager restaura automaticamente
...
/**
 * Retorna true si Speed Freak esta activo para el cliente.
 */
...
{
	g_iSpeedFreak_OriginalHealth[client] = 0;
	// SPD_OnClientDisconnect ya limpia la capa, no necesita llamarse aqui
...
{
	// Solo desactivar si sigue activo (evita doble deactivate si se llamo manualmente)
